...
type SetupRecipeRow = { label?: string | null; required_resource_keys?: string[] | null; optional_resource_keys?: string[] | null; };
type SetupResourceRow = { stable_key?: string | null; label?: string | null; };

const VENUE_STATION_TEMPLATES = new Set([
  "community_thursday_morning_outdoor_v2","community_thursday_morning_coffee_water_v2","community_thursday_morning_rooms_v2",
  "community_thursday_venue_tidy_v1","community_thursday_venue_prep_v1","community_thursday_venue_host_v1",
]);

function isContractorServiceTask(task: AtlasTaskCard) { return task.task_type === "contractor_service_status" || task.metadata?.task_style === "contractor_service_status"; }
function isDirectHarvestTask(task: AtlasTaskCard) { return task.task_type === "owner_decision" && task.metadata?.task_style === "flower_preparation_directive_review" && String(task.metadata?.flower_preparation_directive_review_version ?? "") === "1"; }
function isDecisionSelectorTask(task: AtlasTaskCard) { return task.metadata?.task_style === "decision_selector" && typeof task.metadata?.decision_selector_key === "string" && task.metadata.decision_selector_key.length > 0; }
function isProjectPullTask(task: AtlasTaskCard) { return typeof task.metadata?.project_pull_item_id === "string" && task.metadata.project_pull_item_id.length > 0; }
function isSowCardTask(task: AtlasTaskCard) { return task.task_type === "sowing" && task.action_key === "sow" && (task.metadata?.task_style === "sowing" || task.metadata?.operation_result_membrane === "or3_direct_sow_seed_v1"); }
function isOneOffMowingCardTask(task: AtlasTaskCard) {
  ...
  return task.task_type === "mowing" && task.action_key === "mow" && !clockManaged && task.operation_class === "cut_separate" && typeof task.metadata?.execution_place === "string" && task.metadata.execution_place.trim().length > 0 && task.metadata?.target_cut_height_inches !== null && task.metadata?.target_cut_height_inches !== undefined;
}
function isSprayTreatmentTask(task: AtlasTaskCard) { return task.action_key === "spray" && task.operation_class === "apply_treatment"; }
function isWeedTask(task: AtlasTaskCard) { return task.metadata?.canonical_card_family === "weed" || task.action_key === "weed" || task.task_type === "weed" || task.metadata?.work_route === "weed" || /^weed\b/i.test(task.title.trim()); }
function isSeedInventoryTask(task: AtlasTaskCard) { return task.task_type === "seed_inventory_recount" || task.action_key === "recount_seed_inventory" || task.metadata?.task_style === "seed_inventory_recount"; }
function isBuyerOutreachTask(task: AtlasTaskCard) { return task.metadata?.buyer_outreach_mode === "sales"; }
function isPhoneOutreachTask(task: AtlasTaskCard) { return task.metadata?.task_style === "phone_outreach" || task.metadata?.checklist_mode === "phone_outreach"; }
function isNetworkOutreachTask(task: AtlasTaskCard) { return task.metadata?.network_outreach_master_task === true || task.metadata?.checklist_mode === "network_outreach"; }
function isNetworkInputsTask(task: AtlasTaskCard) { return task.metadata?.network_input_research === true || task.metadata?.network_input_master_task === true || task.metadata?.checklist_mode === "network_input_research" || task.metadata?.task_key === "anna_20260728_call_local_companies_florist_buckets" || task.metadata?.task_key === "anna_20260730_source_free_farm_inputs"; }
function isFarmRoundTask(task: AtlasTaskCard) { return task.task_type === "stewardship_round" && (task.metadata?.farm_round_parent === true || task.metadata?.farm_round_parent === "true"); }
function isCropMoveTask(task: AtlasTaskCard) { return task.task_type === "pot_up" || task.action_key === "pot_up" || (task.task_type === "transplanting" && task.operation_class === "divide_reestablish_belowground"); }
function isResetTask(task: AtlasTaskCard) { return task.operation_class === "clean_restore"; }
function isVenueTask(task: AtlasTaskCard) { const template = task.metadata?.execution_checklist_template_key; return task.task_type === "event_setup" && task.metadata?.collection_zone === "Venue" && typeof template === "string" && VENUE_STATION_TEMPLATES.has(template); }
function isExecutionChecklistTask(task: AtlasTaskCard) { return typeof task.metadata?.execution_checklist_template_key === "string" && task.metadata.execution_checklist_template_key.length > 0; }
function isTransplantReadinessTask(task: AtlasTaskCard) { return task.task_type === "transplant_readiness" || task.metadata?.task_style === "transplant_readiness" || task.metadata?.requires_transplant_readiness_check === true || task.metadata?.requires_transplant_readiness_check === "true"; }
function isFlowerPreparationTask(task: AtlasTaskCard) { return task.task_type === "flower_preparation" || task.metadata?.task_style === "flower_preparation"; }
function isFlowerFulfillmentTask(task: AtlasTaskCard) { return task.task_type === "flower_fulfillment" || task.metadata?.task_style === "flower_fulfillment"; }
function isWeeklyHarvestTask(task: AtlasTaskCard) { return task.task_type === "harvest" && (task.metadata?.weekly_routine === true || task.metadata?.weekly_routine === "true"); }
function isWeeklyFoodHarvestTask(task: AtlasTaskCard) { return task.task_type === "food_harvest" && task.metadata?.result_contract === "weekly_food_harvest_round_v1"; }
function isSiteLayoutTask(task: AtlasTaskCard) { return task.task_type === "site_layout"; }

async function loadWorkerReadiness(taskId: string): Promise<WorkerReadinessResponse> {
  const supabase = await createAtlasServerClient();
  const { data, error } = await supabase.rpc("worker_task_execution_readiness_api_v1", { p_task_id: taskId });
  if (error) { console.error("Task execution readiness failed during Task Focus render.", error); return { ok: false, error: "Task readiness could not be loaded." }; }
...
async function loadSiteLayoutRecipe(task: AtlasTaskCard) {
  ...
  const supabase = await createAtlasServerClient();
  const { data: farm } = await supabase.schema("atlas").from("farms").select("id").eq("stable_key", task.farm_key).limit(1).maybeSingle();
  if (!farm?.id) return { label: null as string | null, tools: [] as string[] };
  const { data: recipe, error: recipeError } = await supabase.schema("atlas").from("action_requirement_templates").select("label, required_resource_keys, optional_resource_keys").eq("farm_id", farm.id).eq("action_type", task.action_key).eq("applies_to_task_type", task.task_type).order("updated_at", { ascending: false }).limit(1).maybeSingle();
  ...
  const { data: resources, error: resourceError } = await supabase.schema("atlas").from("resources").select("stable_key, label").eq("farm_id", farm.id).in("stable_key", keys);
  ...
  return { label: row?.label?.trim() || null, tools: keys.map((key) => labelByKey.get(key) || "").filter(Boolean) };
}
