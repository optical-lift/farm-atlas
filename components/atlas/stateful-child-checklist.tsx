...
function interactivePlantingLogTask(task: AtlasTaskCard) {
  return boolish(meta(task, "planting_log_required"))
    && !boolish(meta(task, "planting_log_auto_capture"));
}

export function statefulChildTask(task: AtlasTaskCard) {
  return boolish(meta(task, "stateful_child"))
    || boolish(meta(task, "planting_log_auto_capture"))
    || boolish(meta(task, "planting_log_required"));
}
...
export default function StatefulChildChecklist({
  ...
  const plantingLogTasks = childTasks
    .filter(interactivePlantingLogTask)
    .sort((a, b) => sortOrder(a) - sortOrder(b) || a.title.localeCompare(b.title));
  const directStatefulTasks = childTasks.filter((task) => !interactivePlantingLogTask(task));
  const orderedTasks = [...directStatefulTasks].sort((a, b) => sortOrder(a) - sortOrder(b) || a.title.localeCompare(b.title));
  const headingSource = orderedTasks[0] ?? plantingLogTasks[0];
  const heading = headingSource ? text(meta(headingSource, "checklist_group_label")) || "Checklist" : "Checklist";
  ...
  return (
    <>
      ...
      <TaskChildChecklist childTasks={plantingLogTasks} onChange={onChange} />
    </>
  );
}