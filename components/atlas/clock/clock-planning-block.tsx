...
export default function ClockPlanningBlock(props:{block:AtlasClockDraftBlock;dateIso:string;top:number;height:number;onMove:(id:string,start:number)=>void;onResize:(id:string,duration:number)=>void;onDecision:(id:string,decision:AtlasClockDraftDecision)=>void;onOverride:(id:string,value:boolean)=>void;onUnplace:(id:string)=>void;}){
 ...
 return <div className={`${styles.timedTask} ${planStyles.planningBlock}`} style={{top:props.top,height:props.height,left:"61px",width:"calc(100% - 69px)",overflow:"visible"}} data-clock-plan-block="true" data-clock-plan-source={block.source} data-clock-plan-decision={block.decision} data-clock-readiness-independent="true" data-timing-class={purple?"potential":block.item.mobility.timingClass} data-warning={warning?"true":"false"}>
  <Link className={planStyles.planningBlockLink} href={block.taskId?href(block.taskId,props.dateIso):`/clock?date=${props.dateIso}`}><small>{purple?"Atlas proposes":"Committed draft"} · {label(block.startMinute)}–{label(block.startMinute+block.durationMinutes)}</small><strong>{block.item.title}</strong><span>{block.durationMinutes}m{block.proposalReason?` · ${block.proposalReason}`:""}</span></Link>
  <button type="button" className={planStyles.dragHandle} aria-label={`Drag ${block.item.title} earlier or later`} onPointerDown={e=>begin(e,"move")} onPointerMove={move} onPointerUp={end} onPointerCancel={end}>↕</button>
  <button type="button" className={planStyles.resizeHandle} aria-label={`Resize duration for ${block.item.title}`} onPointerDown={e=>begin(e,"resize")} onPointerMove={move} onPointerUp={end} onPointerCancel={end}>⋮</button>
  <details className={planStyles.planDetails}><summary>Plan</summary><div className={planStyles.planPopover}>
   <div className={planStyles.planButtons}><button type="button" onClick={()=>props.onMove(block.id,Math.max(0,block.startMinute as number-15))}>−15m</button><button type="button" onClick={()=>props.onMove(block.id,Math.min(1440-block.durationMinutes,(block.startMinute as number)+15))}>+15m</button><button type="button" onClick={()=>props.onResize(block.id,Math.max(5,block.durationMinutes-15))}>Shorter</button><button type="button" onClick={()=>props.onResize(block.id,Math.min(720,block.durationMinutes+15))}>Longer</button></div>
   {purple?<div className={planStyles.planButtons}>{block.decision==="accept"?<button type="button" onClick={()=>props.onDecision(block.id,"pending")}>Undo use</button>:<button type="button" className={planStyles.planPrimary} onClick={()=>props.onDecision(block.id,"accept")}>Use this</button>}<button type="button" onClick={()=>props.onDecision(block.id,"reject")}>Not this</button></div>:<button type="button" className={planStyles.planUnplace} onClick={()=>props.onUnplace(block.id)}>Return to Unplaced</button>}
   {warning?<div className={planStyles.planWarning} data-clock-plan-warning="true"><strong>Timing warning</strong><ul>{block.warnings.map(w=><li key={w.code}>{w.message}</li>)}</ul><button type="button" className={block.overrideWarnings?planStyles.planOverrideActive:""} onClick={()=>props.onOverride(block.id,!block.overrideWarnings)}>{block.overrideWarnings?"Override recorded":"Override warning"}</button></div>:null}
   <small className={planStyles.planState}>{purple?"Purple stays proposed until Commit plan.":"White task truth is unchanged until Commit plan."}</small>
  </div></details>
 </div>;
}
