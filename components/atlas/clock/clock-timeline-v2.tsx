...
export default function ClockTimelineV2(props:{dateIso:string;canManage:boolean;layouts:AtlasClockTaskLayout[];timedCues:Cue[];activeRange:AtlasClockTaskRange|null;selectedToday:boolean;nowMinute:number|null;startHour:number;endHour:number;gridHeight:number;onChanged:()=>Promise<void>;onError:(message:string|null)=>void;}){
 ...
 {props.timedCues.map((item)=>{const minute=clockLocalMinuteOfDay(item.scheduledAt,DEFAULT_ATLAS_FARM_TIME_ZONE);if(minute===null)return null;return <div className={styles.cue} style={{top:offsetForMinute(minute)}} key={item.id} data-clock-timed-cue="true" data-timing-class={item.mobility.timingClass}><i aria-hidden="true"/><div><small>{minuteLabel(minute)} · Cue · {atlasTimingClassLabel(item.mobility)}</small><strong>{item.title}</strong>{item.body?<span>{item.body}</span>:null}</div></div>;})}
