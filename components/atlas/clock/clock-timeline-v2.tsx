...
export default function ClockTimelineV2(props:{dateIso:string;canManage:boolean;layouts:AtlasClockTaskLayout[];timedCues:Cue[];activeRange:AtlasClockTaskRange|null;selectedToday:boolean;nowMinute:number|null;startHour:number;endHour:number;gridHeight:number;onChanged:()=>Promise<void>;onError:(message:string|null)=>void;}){
 ...
 return <section className={styles.gridShell} aria-label="Clock timeline"><header><h2>Time</h2><span>Exact starts + planned spans</span></header><div className={styles.grid} style={{height:props.gridHeight}} data-clock-duration-blocks="true">
 ...
 {props.timedCues.map((item)=>{const minute=clockLocalMinuteOfDay(item.scheduledAt,DEFAULT_ATLAS_FARM_TIME_ZONE);if(minute===null)return null;return <div className={styles.cue} style={{top:offsetForMinute(minute)}} key={item.id} data-clock-timed-cue="true"><i aria-hidden="true"/><div><small>{minuteLabel(minute)} · Cue</small><strong>{item.title}</strong>{item.body?<span>{item.body}</span>:null}</div></div>;})}
 {props.layouts.map((layout)=>{const item=layout.item;const height=layout.span.minutes?Math.max(38,(layout.span.minutes/60)*HOUR_HEIGHT-2):42;const available="calc(100% - 69px)";const left=layout.laneCount===1?"61px":`calc(61px + (${available} * ${layout.laneIndex} / ${layout.laneCount}))`;const width=layout.laneCount===1?"calc(100% - 69px)":`calc((${available} / ${layout.laneCount}) - 4px)`;return <div className={styles.timedTask} style={{top:offsetForMinute(layout.startMinute),height,left,width,overflow:"visible"}} key={item.id} data-clock-timed-task="true" data-clock-planned-span={layout.span.minutes?"true":"false"} data-active={props.activeRange?.item.id===item.id?"true":"false"} data-conflict={layout.conflict?"true":"false"}><Link style={{maxHeight:"100%",overflow:"hidden",paddingRight:props.canManage?28:0}} href={item.taskId?taskHref(item.taskId,props.dateIso):`/clock?date=${props.dateIso}`}><small>{layout.span.minutes?`${minuteLabel(layout.startMinute)}–${minuteLabel(layout.endMinute)}`:`${minuteLabel(layout.startMinute)} · Start only`}</small><strong>{item.title}</strong>{item.location?<span>{item.location}</span>:null}{layout.conflict?<span className={styles.conflictNote}>Overlaps another planned block</span>:null}</Link><ClockOwnerControls item={item} dateIso={props.dateIso} canManage={props.canManage} onChanged={props.onChanged} onError={props.onError} showTime compact/></div>;})}
 </div></section>;
}
