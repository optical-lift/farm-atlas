...
export default function ClockTimelineV2(props:{dateIso:string;canManage:boolean;layouts:AtlasClockTaskLayout[];proposals:AtlasClockProposalBlock[];timedCues:Cue[];dayReservations:AtlasClockReservation[];activeRange:AtlasClockTaskRange|null;selectedToday:boolean;nowMinute:number|null;startHour:number;endHour:number;gridHeight:number;onChanged:()=>Promise<void>;onError:(message:string|null)=>void;}){
 ...
 return <section className={styles.gridShell} aria-label="Clock timeline"><header><h2>Time</h2><span>{props.proposals.length ? "White = committed · purple = proposed" : "Exact starts + planned spans"}</span>{props.canManage?<div style={{marginLeft:"auto",display:"flex",gap:6}}><button type="button" onClick={()=>setRoutineManagerOpen(true)} style={{border:"1px solid rgba(88,87,111,.18)",borderRadius:999,background:"#fff",padding:"7px 10px",minHeight:38,fontSize:10,fontWeight:800}}>Routines</button><button type="button" onClick={()=>setCreateMinute(12*60)} style={{border:"1px solid rgba(88,87,111,.18)",borderRadius:999,background:"#fff",padding:"7px 10px",minHeight:38,fontSize:10,fontWeight:800}}>+ Fixed time</button></div>:null}</header><div className={styles.grid} style={{height:props.gridHeight}} data-clock-duration-blocks="true" data-clock-create-reservation={props.canManage?"tap-open-space":undefined} onClick={createAtGridPoint}>
 ...
 </div>{createMinute!==null?<ReservationEditor dateIso={props.dateIso} defaultStartMinute={createMinute} onClose={()=>setCreateMinute(null)}/>:null}{routineManagerOpen?<FixedRoutineManager dateIso={props.dateIso} onClose={()=>setRoutineManagerOpen(false)}/>:null}</section>;
}
