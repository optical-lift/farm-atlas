...
export default function ClockPlanningTimeline(props:{
  dateIso:string;canManage?:boolean;blocks:AtlasClockDraftBlock[];timedCues:Cue[];dayReservations:AtlasClockReservation[];selectedToday:boolean;nowMinute:number|null;
  ...
  return <section className={styles.gridShell} aria-label="Clock planning timeline" data-clock-plan-timeline="true">
    ...
    <div className={styles.grid} style={{height:props.gridHeight}} data-clock-create-reservation={canManage?"tap-open-space":undefined} onClick={createAtGridPoint}>
      ...
      {props.dayReservations.filter((reservation)=>reservation.source!=="timed_cue").map((reservation)=><ClockReservationBlock key={reservation.id} dateIso={props.dateIso} canManage={canManage} reservation={reservation} startHour={props.startHour}/>)}
