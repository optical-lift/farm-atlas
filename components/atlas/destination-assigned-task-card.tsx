...
function destinationSubtitle(task: AssignedTaskExecutionShellProps["task"]) {
  ...
  if (action && value.toLowerCase().startsWith(`${action.toLowerCase()} `)) value = value.slice(action.length).trim();
  if (subject) {
    const atSubject = new RegExp(`\\s+at\\s+${subject.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}`, "i");
    value = value.replace(atSubject, "");
  }
  value = value.trim();
