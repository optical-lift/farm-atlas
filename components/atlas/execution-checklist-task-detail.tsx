...
function addDays(dateIso: string, days: number) { const date = new Date(`${dateIso}T12:00:00`); date.setDate(date.getDate() + days); return date.toISOString().slice(0, 10); }
function requestError(data: ChecklistResponse) { if (data.details) return data.details; if (typeof data.error === "string") return data.error; return data.error?.message || "Atlas could not update the checklist."; }
...
function metadataText(task: AtlasTaskCard, key: string) { const value = task.metadata?.[key]; return typeof value === "string" && value.trim() ? value.trim() : null; }
...
export default function ExecutionChecklistTaskDetail({ task, assignee }: Props) {
  ...
  const sections = useMemo(() => {
    ...
    for (const item of checklist?.items ?? []) {
      ...
      if (!section) { section = { key: item.sectionKey, label: item.sectionLabel, items: [] }; ordered.push(section); }
  ...
  async function toggle(item: ChecklistItem) {
    ...
    try {
      setSavingItem(item.itemKey); setMessage(null);
      setChecklist((current) => current ? { ...current, items: current.items.map((candidate) => candidate.itemKey === item.itemKey ? { ...candidate, checked: nextChecked } : candidate), completeCount: current.completeCount + (nextChecked ? 1 : -1), ready: current.items.every((candidate) => candidate.itemKey === item.itemKey ? nextChecked || !candidate.required : candidate.checked || !candidate.required) } : current);
  ...
  async function transition(outcome: "done" | "partial" | "blocked" | "changed_plan" | "not_relevant", note = "") {
    try {
      setSavingOutcome(outcome); setMessage(null);
      await postAtlasTaskTransition({ taskId: task.task_id, transition: outcome, note, reason: note, laneKey: task.action_key || undefined, workKey: task.action_key || undefined, payload: { assigneeKey: assignee.key, completion_source: outcome === "done" ? "execution_checklist" : "task_card", checklistComplete: checklist?.ready === true } });
      if (outcome === "done" || outcome === "changed_plan" || outcome === "not_relevant") { window.location.assign(assignee.listPath); return; }
      setUnfinishedOpen(false); setMessage("Saved.");
  ...
  async function reschedule(targetDate: string | null, reason: string, scheduleIntent?: string) {
    try {
      setSavingOutcome("reschedule"); setMessage(null);
  ...
  return (
    <main className="atlas-phone-shell atlas-home-shell atlas-task-page-shell">
      <style>{`
        ...
        .atlas-execution-checklist__loading, .atlas-execution-checklist__completion-note { padding:18px 22px; color:#777; font-size:.88rem; line-height:1.35; }
        ...
        <div className="atlas-task-page-body">
          <article className="atlas-task-page-active atlas-task-ticket-card atlas-dominion-task-card">
            ...
            <section className="atlas-execution-checklist" aria-label={checklistTitle}>
              <header className="atlas-execution-checklist__head"><div><span>{checklistKicker}</span><strong>{checklistTitle}</strong></div><div className="atlas-execution-checklist__progress">{checklist ? `${checklist.completeCount} / ${checklist.totalCount}` : "Loading"}</div></header>
              {!checklist ? <p className="atlas-execution-checklist__loading">Loading checklist…</p> : sections.map((section) => (
                <section className="atlas-execution-checklist__section" key={section.key}>
                  <h2>{section.label}</h2>
                  <div className="atlas-execution-checklist__items">{section.items.map((item) => (
                    <button type="button" className={`atlas-execution-checklist__item${item.checked ? " is-checked" : ""}`} key={item.itemKey} aria-pressed={item.checked} disabled={busy} onClick={() => void toggle(item)}>
                      <span className="atlas-execution-checklist__mark" aria-hidden="true">{item.checked ? "✓" : ""}</span><span>{item.label}</span>
                    </button>
                  ))}</div>
                </section>
              ))}
              {checklist && !checklist.ready ? <p className="atlas-execution-checklist__completion-note">Finish the required lines before marking the task done.</p> : null}
            </section>

            <footer className="atlas-task-result-footer">
              <TaskPrimaryResultControls busy={busy} doneBusy={savingOutcome === "done"} doneDisabled={checklist?.ready !== true} unfinishedOpen={unfinishedOpen} onToggleUnfinished={() => setUnfinishedOpen((open) => !open)} onDone={() => void transition("done")}>
              ...
              <details className="atlas-task-more-outcomes">
                ...
                <div className="atlas-task-more-outcomes-body">
                  ...
                  <div className="atlas-task-more-outcomes-grid">
                    <button type="button" disabled={busy} onClick={() => void reschedule(null, "Moved to next Elm Farm calendar day", "next_day")}>Tomorrow</button>
                    <button type="button" disabled={busy} onClick={() => void reschedule(addDays(todayIso(), 7), "Moved to next week")}>Next week</button>
                    <button type="button" disabled={busy} onClick={() => { const date = window.prompt("Pick a date (YYYY-MM-DD)", task.due_date || todayIso())?.trim(); if (date) void reschedule(date, "Rescheduled from checklist task"); }}>Pick a date</button>
                  </div>
