...
function OwnerButtons({ onRoutine, onAdd }: { onRoutine: () => void; onAdd: () => void }) {
  return <div style={{ display: "flex", gap: 6 }}>
    <button type="button" onClick={onRoutine} style={{ border: "1px solid rgba(88,87,111,.14)", borderRadius: 999, background: "white", padding: "7px 10px", minHeight: 38, fontSize: 9, fontWeight: 800 }}>Routines</button>
    <button type="button" onClick={onAdd} style={{ border: "1px solid rgba(88,87,111,.14)", borderRadius: 999, background: "white", padding: "7px 10px", minHeight: 38, fontSize: 9, fontWeight: 800 }}>+ Add</button>
  </div>;
}
...
export default function DayFixedTimes({ dateIso }: { dateIso: string }) {
  ...
  const routineManager = routineManagerOpen ? <FixedRoutineManager dateIso={dateIso} onClose={() => setRoutineManagerOpen(false)} /> : null;
  ...
  if (!reservations.length) return canManage ? (
    <section data-day-fixed-times="true" style={{ margin: "0 0 12px", padding: "10px 12px", border: "1px solid rgba(88,87,111,.1)", borderRadius: 12, background: "rgba(248,248,251,.72)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <div><small style={{ display: "block", color: "#777", textTransform: "uppercase", letterSpacing: ".08em", fontSize: 8 }}>Today’s fixed times</small><strong style={{ fontSize: 12 }}>No occupied life time recorded</strong></div>
        <OwnerButtons onRoutine={() => setRoutineManagerOpen(true)} onAdd={() => setEditing("new")} />
      </div>
      {editor}
      {routineManager}
    </section>
  ...
  return (
    <section data-day-fixed-times="true" aria-label="Today’s fixed times" style={{ margin: "0 0 12px", padding: "10px 12px", border: "1px solid rgba(88,87,111,.1)", borderRadius: 12, background: "rgba(248,248,251,.72)" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 7 }}>
        <div><small style={{ display: "block", color: "#777", textTransform: "uppercase", letterSpacing: ".08em", fontSize: 8 }}>Today’s fixed times</small><strong style={{ fontSize: 12 }}>Real time outside the task list</strong></div>
        {canManage ? <OwnerButtons onRoutine={() => setRoutineManagerOpen(true)} onAdd={() => setEditing("new")} /> : null}
      </header>
