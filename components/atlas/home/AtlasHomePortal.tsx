...
import type { AtlasViewer } from "@/lib/atlas/viewer";
...
const defaultSnapshot: AtlasFarmSnapshot = {
  totalBeds: 0,
  growingBeds: 0,
  activeSqft: 0,
  sowingsLogged: 0,
...
function isTaskDone(card: AtlasTaskCard) {
  return card.status === "done"
    || card.task_outcomes?.[0]?.outcome === "done"
    || atlasMetaString(card, "checklist_status") === "done";
}
...
function weekCountForRange(cards: AtlasTaskCard[], startIso: string, endIso: string) {
  return cards
    ...
    .filter((task) => task.due_date && task.due_date >= startIso && task.due_date <= endIso)
    .length;
}
...
function panelTitle(panel: HomePanel) {
  return panel === "closeout" ? "Closeout" : "Atlas";
}
...
function launchItemSignature(item: HomeLaunchItem, stepCounts: Map<string, number>) {
  if (item.kind === "collection") {
    return `collection:${item.collection.key}:${item.collection.dueCount}:${item.collection.doneRecentCount}:${item.collection.notReadyCount}`;
  }
...
function TaskLaunchHero({ cards, loading }: { cards: AtlasTaskCard[]; loading: boolean }) {
  ...
  if (loading && cards.length === 0) {
    return (
      <AtlasCard
        variant="purple"
        className="atlas-home-box atlas-home-box-purple atlas-home-task-hero atlas-task-controller atlas-daily-run-sheet empty"
      >
  ...
  return (
    ...
    >
      ...
      {heroItems.length === 0 ? (
        ...
        <div
          className="atlas-run-sheet-grid atlas-route-sheet-grid"
          data-task-forward-signature={heroItems.map((item) => launchItemSignature(item, stepCounts)).join("|")}
        >
          {heroItems.map((item) => {
            if (item.kind === "collection") {
              return (
                <Link
                  ...
                  href={item.collection.href}
                  className="atlas-run-sheet-box atlas-route-sheet-box atlas-task-forward-box atlas-work-collection-forward-box"
                  data-work-collection-key={item.collection.key}
                >
            ...
            return (
              <Link
                ...
                className={`atlas-run-sheet-box atlas-route-sheet-box atlas-task-forward-box${item.overdue ? " atlas-overdue-forward-box" : ""}`}
                data-single-task-id={card.task_id}
              >
                <small>{item.overdue ? `Overdue · ${prettyDate(card.due_date)}` : display.action || routeLabel}</small>
...
function HomeFooterBar({ summary, today, onOpen }: { summary: AtlasCloseoutSummary | undefined; today: string; onOpen: () => void }) {
  ...
  return (
    <AtlasFooterActions className="atlas-home-footer-row">
      ...
      <Link
        href="/production"
        className="atlas-home-frost-countdown"
        aria-label={`Open production calendar · ${frostDays} days until first frost target on November 1`}
      >
...
export default function AtlasHomePortal({ viewer }: AtlasHomePortalProps) {
  ...
  async function loadCards() {
    try {
      ...
      setCards((response.taskCards ?? []).sort((a, b) => taskSortValue(a).localeCompare(taskSortValue(b))));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function afterFieldLogSaved() {
    await loadRegistry();
    await loadSnapshot();
    await loadCloseout();
  }
        ...
        <section className="atlas-task-focus-overlay" role="dialog" aria-modal="true">
          <div className="atlas-task-focus-phone">
            <div className="atlas-task-focus-topbar">
              <div><strong>{panelTitle(openPanel)}</strong></div>
  ...
  return (
    <AtlasAppShell
      ...
      data-atlas-home-portal="shared"
      data-atlas-viewer-role={viewer.role}
      data-atlas-viewer-worker={viewer.workerKey ?? "unassigned"}
      afterFrame={afterFrame}
    >
      <AtlasTopBar
        title={viewer.farmName}
        status={<span className="atlas-weather-line">{weatherLabel}</span>}
        action={(
          <button
            type="button"
            className="atlas-note-plus"
            aria-label="Document work"
            onClick={() => {
              setOpenPanel(null);
