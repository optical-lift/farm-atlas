...
function HomeFooterBar({ summary, today, onOpen }: { summary: AtlasCloseoutSummary | undefined; today: string; onOpen: () => void }) {
  ...
  return <div className="atlas-home-footer-row"><Link href="/closeout" className="atlas-home-closeout-footer-link" onClick={onOpen}><span>Closeout</span><em>{summary ? `${summary.counts.objectEvents} records · ${summary.counts.openTasks} open` : `Review · ${prettyDate(today)}`}</em></Link><Link href="/production" className="atlas-home-frost-countdown" aria-label={`Open production calendar · ${frostDays} days until first frost target on November 1`}><span>First frost</span><em>{frostDays} days · Nov 1</em></Link></div>;
}
