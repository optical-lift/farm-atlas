...
export default function AtlasUniversalHome({ home, dayOverview, farmSeasons, farmHandMode = false, hideTimeNavigation = false }: AtlasUniversalHomeProps) {
  const [weatherLabel, setWeatherLabel] = useState<string | null>(null); const todayIso = home.window.doneDate;
  ...
  return <AtlasAppShell className="atlas-home-shell" frameClassName={styles.frame} data-atlas-home-portal="universal-v2" data-atlas-has-farm-scope={home.viewer.hasFarmScope ? "true" : "false"} data-atlas-has-organization-scope={home.viewer.hasOrganizationScope ? "true" : "false"}><AtlasTopBar title={home.title} status={<span className="atlas-weather-line">{headerStatus}</span>}/><div className={styles.home}><div className={styles.todayStack}><AtlasCard variant="purple" className={styles.hero} ariaLabelledBy="atlas-today-title"><div className={styles.heroHead}><div className={styles.heroIdentity}><span>{coverLabel}</span><em id="atlas-today-title">{farmHandMode ? prettyDay(todayIso) : prettyDay(todayIso)}</em></div><span className={styles.heroStatus}><b>{progressLabel}</b>{carryForwardLabel ? <em>{carryForwardLabel}</em> : null}</span></div>{visibleMoves.length ? <div className={styles.heroGrid} data-task-count={visibleMoves.length} data-atlas-home-task-board="true">{visibleMoves.map((move, index) => { const taskId = taskIdFromMoveKey(move.key); const taskPosition = move.kind === "farm_task" ? index === 0 ? "current" : index === 1 ? "next" : "later" : "oversight"; return <article key={move.key} className={styles.heroMove} data-state={move.state} data-position={taskPosition}><Link href={move.href} className={styles.heroMoveBody} data-single-task-id={taskId || undefined}><small>{farmHandMode ? "Next at Elm" : move.category}</small><strong>{move.title}</strong><span>{move.scopeLabel}{move.meta ? ` · ${move.meta}` : ""}</span>{move.detail ? <em>{move.detail}</em> : null}</Link>{index === 0 && move.kind === "farm_task" ? <Link className={styles.heroAction} href={move.href}>{move.state === "blocked" ? "See what’s in the way" : farmHandMode ? "Start" : "Finish"}</Link> : null}</article>; })}</div> : <div className={styles.heroEmpty}><strong>{farmHandMode ? "Farm work is caught up for today" : hasCarryForward ? overdueLabel : dayOverview.personalScope ? "No personal work is due today" : "The day is clear"}</strong><em>{farmHandMode ? "Atlas will put the next useful move here when there is one." : hasCarryForward ? "Open the day overview to work through the oldest unfinished tasks." : "Open Work to inspect the next planned day."}</em></div>}</AtlasCard>{hideTimeNavigation ? null : <HomeTimeRail home={home}/>}</div>{farmHandMode ? null : <NeedsYou home={home}/>}<TheFarms home={home} farmSeasons={farmSeasons}/></div></AtlasAppShell>;
}
