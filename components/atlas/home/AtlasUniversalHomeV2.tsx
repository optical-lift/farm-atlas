...
import FarmConditionsPanel, { type FarmConditionsResponse } from "@/components/atlas/home/FarmConditionsPanel";
import { AtlasAppShell, AtlasCard, AtlasTopBar } from "@/components/atlas/ui/AtlasPrimitives";
import {
  atlasFarmWeekStartMonday,
  atlasShiftFarmDate,
...
type FarmConditionsCollectionResponse = { ok: boolean; conditions?: FarmConditionsResponse[]; error?: string };
...
function dayRail(items: AtlasUniversalDatedItem[], todayIso: string) {
  const startIso = atlasFarmWeekStartMonday(todayIso);
  return Array.from({ length: 7 }, (_, index): DayRailItem => {
    const dateIso = atlasShiftFarmDate(startIso, index);
...
function HomeTimeRail({ home }: { home: AtlasUniversalHomeModel }) {
  ...
  const previousWeek = atlasShiftFarmDate(weekStart, -7);
  const previousWeekEnd = atlasShiftFarmDate(previousWeek, 6);
  return (
    <section className={styles.timeRail} aria-label="Days in this week" data-atlas-home-time-rail="true">
      <div className={styles.days} aria-label="Open a day in this week">
        {days.map((day) => (
          ...
          >
            <small>{day.weekday}</small><strong>{day.day}</strong><em>{dayMarker(day)}</em>
          </Link>
        ))}
      </div>
  ...
  return (
    <AtlasCard as="section" className={styles.lens} ariaLabelledBy="atlas-home-needs-title">
      <header className={styles.lensHeader}>
        <div><span>Owner lane</span><h2 id="atlas-home-needs-title">Needs you</h2></div>
      ...
      <div className={styles.lensList}>
        {items.map((item, index) => (
          <Link key={`${item.attentionId ?? item.projectId}-${index}`} href={`/project/${encodeURIComponent(item.projectId)}`}>
            <div><small>{titleCase(item.kind)}</small><strong>{item.title}</strong><span>{item.detail || item.projectTitle}</span></div>
...
function TheFarms({ home, farmSeasons, conditionsByFarmId, onConditionsReload }: {
  home: AtlasUniversalHomeModel;
  farmSeasons: Record<string, AtlasHomeFarmSeasonProfile>;
  conditionsByFarmId: Record<string, FarmConditionsResponse>;
  onConditionsReload: () => void;
}) {
  return (
    <section className={styles.farmsSection} aria-label="Farm seasons">
      <div className={styles.farmCards}>
        {home.farms.map((farm) => {
          ...
          const conditions = conditionsByFarmId[farm.farmId];
          const runway = frostRunway(home.window.doneDate, season);
          const activePercent = snapshot.totalBeds > 0 ? Math.min(100, Math.round((snapshot.growingBeds / snapshot.totalBeds) * 100)) : 0;
          const roleLabel = farm.role === "owner" ? "Stewarding" : "Working at";
          const identityLine = season?.locationLabel ? `${roleLabel} · ${season.locationLabel}` : roleLabel;
          return (
            <article className={styles.farmCard} key={farm.farmId} data-farm-id={farm.farmId} data-has-growing-beds={snapshot.growingBeds > 0 ? "true" : "false"}>
              <header className={styles.farmCardHead}>
                <div><small>{identityLine}</small><h3>{farm.farmName}</h3></div>
                <span className={styles.frostBadge} data-frost-known={runway.known ? "true" : "false"}><b>{runway.known ? runway.days : "?"}</b><em>{runway.known ? "days to frost" : "frost date unknown"}</em></span>
              </header>
              {conditions ? <FarmConditionsPanel conditions={conditions} onReload={onConditionsReload} /> : null}
              <div className={styles.farmMetrics}>
                <div><b>{formatCount(snapshot.growingBeds)}</b><span>beds growing</span></div>
                <div><b>{formatCount(snapshot.activeSqft)}</b><span>sq ft active</span></div>
                <div><b>{formatCount(snapshot.stemsLogged)}</b><span>stems this year</span></div>
              </div>
              {snapshot.totalBeds > 0 ? (
                <div className={styles.bedProgress}>
                  <div><span>{snapshot.growingBeds} of {snapshot.totalBeds} mapped beds growing</span><b>{activePercent}%</b></div>
...
export default function AtlasUniversalHome({
  ...
  farmHandMode = false,
  fixtureOnly = false,
  fixtureWeatherLabel = null,
  fixtureConditionsByFarmId = {},
  ...
  useEffect(() => {
    if (fixtureOnly || !home.activeFarm || home.activeFarm.farmId !== home.viewer.activeFarmId) return;
    let active = true;
    fetch("/api/atlas/weather", { headers: { Accept: "application/json" }, cache: "no-store" })
      .then((response) => response.json())
      .then((data: WeatherResponse) => { if (active) setWeatherLabel(data.ok && data.label ? data.label : null); })
      .catch(() => { if (active) setWeatherLabel(null); });
    return () => { active = false; };
  }, [fixtureOnly, home.activeFarm, home.viewer.activeFarmId]);
  ...
  useEffect(() => {
    ...
    fetch("/api/atlas/farm-conditions/all", { headers: { Accept: "application/json" }, cache: "no-store" })
      ...
      .then((conditions) => {
        if (!active) return;
        setConditionsByFarmId(Object.fromEntries(conditions.map((entry) => [entry.farm.id, entry])));
      })
      .catch(() => { if (active) setConditionsByFarmId({}); });
    return () => { active = false; };
  ...
  const progressLabel = farmHandMode
    ? linedUp > 0 ? `${linedUp} more ${linedUp === 1 ? "thing" : "things"} lined up today` : dayOverview.openCount > 0 ? "This is the work in front of you" : "Farm work is caught up for today"
    : dayOverview.plannedTotal > 0
      ? dayOverview.personalScope ? `${dayOverview.dealtCount} of ${dayOverview.plannedTotal} personal tasks dealt with · ${dayOverview.openCount} open` : `${dayOverview.dealtCount} of ${dayOverview.plannedTotal} dealt with · ${dayOverview.openCount} open`
  ...
  return (
    <AtlasAppShell
      ...
      data-atlas-home-data-mode={fixtureOnly ? "fixture" : "live"}
    ...
    >
      <AtlasTopBar title={home.title} status={<span className="atlas-weather-line">{headerStatus}</span>} />
      <div className={styles.home}>
        <div className={styles.todayStack}>
          <AtlasCard variant="purple" className={styles.hero} ariaLabelledBy="atlas-today-title">
            <div className={styles.heroHead}>
              <div className={styles.heroIdentity}><span>{coverLabel}</span><em id="atlas-today-title">{prettyDay(todayIso)}</em></div>
              <span className={styles.heroStatus}><b>{progressLabel}</b>{carryForwardLabel ? <em>{carryForwardLabel}</em> : null}</span>
            </div>
            {visibleMoves.length ? (
              <div className={styles.heroGrid} data-task-count={visibleMoves.length} data-atlas-home-task-board="true">
                {visibleMoves.map((move, index) => {
                  const taskId = taskIdFromMoveKey(move.key);
                  const taskPosition = move.kind === "farm_task" ? index === 0 ? "current" : index === 1 ? "next" : "later" : "oversight";
                  return (
                    <article key={move.key} className={styles.heroMove} data-state={move.state} data-position={taskPosition}>
                      <Link href={move.href} className={styles.heroMoveBody} data-single-task-id={taskId || undefined}>
                        <small>{farmHandMode ? "Next at Elm" : move.category}</small>
                        <strong>{move.title}</strong>
                        <span>{move.scopeLabel}{move.meta ? ` · ${move.meta}` : ""}</span>
                        {move.detail ? <em>{move.detail}</em> : null}
                      </Link>
                      {index === 0 && move.kind === "farm_task" ? <Link className={styles.heroAction} href={move.href}>{move.state === "blocked" ? "See what’s in the way" : farmHandMode ? "Start" : "Finish"}</Link> : null}
                    </article>
                  );
                })}
              </div>
              ...
              <div className={styles.heroEmpty}>
                <strong>{farmHandMode ? "Farm work is caught up for today" : hasCarryForward ? overdueLabel : dayOverview.personalScope ? "No personal work is due today" : "The day is clear"}</strong>
                <em>{farmHandMode ? "Atlas will put the next useful move here when there is one." : hasCarryForward ? "Open the day overview to work through the oldest unfinished tasks." : "Open Work to inspect the next planned day."}</em>
              </div>
            )}
          </AtlasCard>
          <HomeTimeRail home={home} />
        </div>
        {farmHandMode ? null : <NeedsYou home={home} />}
        <TheFarms
          home={home}
          farmSeasons={farmSeasons}
          conditionsByFarmId={conditionsByFarmId}
          onConditionsReload={() => { if (!fixtureOnly) setConditionsReloadVersion((value) => value + 1); }}
        />
      </div>
    </AtlasAppShell>
  );
