...
export default function DayTrailSummary({ completed, total, blocked, loading = false, compact = false }: DayTrailSummaryProps) {
  // The caller computes these counts from the exact Day collection it renders.
  // Do not replace them with a second due-date-based candidate universe: doing so
  // lets the header disagree with the visible Day (for example, 7 of 10 while
  // only three placements are actually on the day).
