...
Atlas can receive authorized observations, preserve provenance, reconcile source identities around durable subjects, reconcile events and corrections, invoke the correct domain contract, and expose one trustworthy relationship history without requiring callers to know internal tables. Elm Local is separated from Atlas Core and offered through the optional Smart Contacts integration.
...
2. **#788 — Evidence-first identity reconciliation.** Establish tenant-scoped identity subjects, source-record references, identity assertions/claims, review/adjudication, correction semantics, and Party/Person/Organization/Place read projections. Do not build a privileged canonical Party directory.
3. **#789 — Atlas Receive v1.** Build the common evidence-intake and reconciliation spine that feeds identity/event/domain reconciliation.
4. **#790 — Canonical relationship timeline.** Build current-position, timeline, open-loop, and provenance read models over reconciled subjects/relationships.
...
7. **#793 — Elm Farm reality backfill.** Progressively bind scattered buyer/outreach/source history into the reconciliation model with original provenance retained; do not bulk-copy legacy rows into a new canonical directory.
...
Flowerama/Recinna, House of Flowers current-buyer uncertainty, Rose Among Thorns owner vs purchasing authority, same-name explicit non-match, mistaken-merge correction, and Smart Contacts discovery reconciled against existing Atlas evidence.
...
2. source identities can reconcile to durable tenant-scoped subjects without making any source row canonical authority;
3. Party/Person/Organization/Place are usable projections over reconciled identity evidence;
4. integrations attach as provider/source evidence with declared authority;
5. evidence, reconciled events, and projections are structurally distinct;
6. one canonical relationship timeline/current-position read contract exists;
7. Smart Contacts / Elm Local is optional and removable;
8. Rounds preserve planned intent and actual encounter history;
9. representative Elm Farm history is progressively bound with provenance and correction semantics intact;
10. ordinary assistant writes no longer select internal storage destinations;
11. legacy reconstruction/write paths are retired only after parity is demonstrated.
