...
test("Daily Hand candidates retain canonical task links and truthful overdue state", () => {
  assert.match(dailyHand, /href: `\/task-focus\/\$\{encodeURIComponent\(card\.task_id\)\}/);
  assert.match(dailyHand, /state: overdue \? "attention" : "ready"/);
  assert.match(dailyHand, /display\.detail \|\| card\.unlock_text \|\| card\.note/);
});
