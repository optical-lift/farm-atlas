...
test("Farm Round owns its route instrument inside canonical task-card geometry", () => {
  assert.match(farmRound, /AtlasTaskCardFrame/);
  assert.match(farmRound, /canonical-card-geometry-v1/);
  assert.match(farmRound, /roundStyles\.route/);
  assert.match(farmRound, /roundStyles\.stop/);
  assert.match(farmRound, /roundStyles\.toggle/);
