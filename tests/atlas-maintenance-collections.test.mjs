...
test("Day keeps canonical maintenance jobs in the mixed timeline while the compact drawer stays recovery-only", () => {
  ...
  assert.match(source, /windowedTimeline\(visibleTimelineGroups, isFutureDay \? filteredFutureAutomaticItems : \[\]\)/);
  ...
  assert.match(source, /useAtlasWorkerDayProjection\(dateIso\)/);
  assert.match(source, /taskCards: tasks/);
