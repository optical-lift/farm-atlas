...
test("elapsed clocks satisfy the existing planned-work gate instead of creating a second task system", () => {
...
test("the clock advances frequently enough for elapsed farm processes", () => {
  ...
  assert.match(core, /'\*\/5 \* \* \* \*'/);
  assert.match(core, /select atlas\.advance_task_dependency_clocks_v1\(\)/);
});
