...
const migration = readFileSync(join(root, "supabase/migrations/20260816124500_worker_weekly_farm_contract_v1.sql"), "utf8");
...
test("weekend capacity never silently becomes normal planned capacity", () => {
  ...
  assert.match(migration, /when v_capacity_class='planned' then least\(v_target,v_usable\) else 0 end/i);
  assert.match(surface, /Saturday\/Sunday capacity is never counted as normal planned capacity/i);
});

test("weekly work preserves canonical temporal provenance instead of promoting task target dates", () => {
  assert.match(migration, /earliest_lawful_date/i);
  assert.match(migration, /latest_lawful_date/i);
  assert.match(migration, /hard_finish_date/i);
  assert.match(migration, /canonical_lawful_window/i);
  assert.match(migration, /task_target_only/i);
});
...
test("internal weekly functions stay service-role-only while authenticated callers use scoped wrappers", () => {
  assert.match(migration, /revoke all on function atlas\.worker_week_day_capacity_v1\(uuid,uuid,date\) from public,anon,authenticated/i);
  assert.match(migration, /grant execute on function atlas\.worker_week_day_capacity_v1\(uuid,uuid,date\) to service_role/i);
  assert.match(migration, /revoke all on function atlas\.worker_weekly_farm_contract_v1\(uuid,uuid,date\) from public,anon,authenticated/i);
