...
test("5K notebook keeps measurement, rhythm, and response policy as separate acceptances", async () => {
  ...
  assert.match(client, /01 · measurement/);
  assert.match(client, /02 · rhythm/);
  assert.match(client, /03 · response policy/);
  assert.match(client, /The observation never invents this rule/);
});
...
test("notebook exposes provenance while retaining the generic governed capture instruments", async () => {
  const client = await read("app/owner/life/PersonLifeCaptureClient.tsx");
  ...
  assert.match(client, /run Evidence/);
