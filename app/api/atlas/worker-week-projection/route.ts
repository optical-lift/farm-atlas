...
function privateJson(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "X-Atlas-Read-Path": "worker-week-projection-v1",
    },
  });
}
...
export async function GET(request: Request) {
  ...
  if (span < 0 || span > 13) {
    return privateJson({ ok: false, error: "Worker week projection supports a 1–14 day range." }, 400);
  }
  ...
  try {
    ...
    const projection = await readWorkerWeekProjection(
      effective.farmId,
      effectiveMembershipId,
      projectionStart,
      dayCount,
    ...
    console.error("Atlas Worker week projection read failed:", error);
    return privateJson({ ok: false, error: "Worker week planning could not be loaded." }, 500);
  }
}
