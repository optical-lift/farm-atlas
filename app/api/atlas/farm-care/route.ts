...
export async function GET() {
  ...
  const { data, error } = await supabase.rpc("farm_care_summary_v1", {
  ...
  if (error) {
    ...
    console.error("Atlas Farm Care read failed:", error);
    return privateJson({ ok: false, error: "Atlas Farm Care read failed." }, 500);
  }
