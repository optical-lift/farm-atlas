...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (dueDate === undefined) {
    return privateJson({ ok: false, error: "Due date must use YYYY-MM-DD." }, 400);
  }

  const supabase = await createAtlasServerClient();
  const { data, error } = await supabase.rpc("create_project_task_v1", {
  ...
  return privateJson({ ok: true, taskId: data }, 201);
}
