...
function privateJson(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "X-Atlas-Read-Path": "worker-day-choreography-v1",
    },
  });
}
...
export async function GET(request: Request) {
  ...
  try {
    const result = await readOwnerWorkerDayChoreography(date as string);
    return privateJson({ ok: true, date, ...result });
  } catch (error) {
    console.error("Atlas Day choreography read failed:", error);
    return privateJson({ ok: false, error: "Atlas could not resolve this Day choreography." }, 500);
  }
}
