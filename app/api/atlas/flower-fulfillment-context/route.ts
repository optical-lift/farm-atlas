...
type OrderRow = { id: string; farm_id: string; customer_label: string | null; sales_channel: string; event_key: string | null; sale_date: string; fulfillment_mode: string; fulfillment_due_date: string | null; fulfillment_due_time: string | null; total_amount: number | string; note: string | null };
...
export async function GET(request: Request) {
  ...
  const [orderResult, linesResult, fulfillmentResult] = await Promise.all([
    supabase.from("flower_sale_orders").select("id, farm_id, customer_label, sales_channel, event_key, sale_date, fulfillment_mode, fulfillment_due_date, fulfillment_due_time, total_amount, note").eq("id", saleOrderId).eq("farm_id", task.farm_id).limit(1).maybeSingle(),
