...
function privateJson(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "X-Atlas-Mutation": "owner-clock-time-v1",
    },
  });
}
...
export async function POST(request: NextRequest) {
  ...
  if (request.headers.get("x-atlas-intent") !== "owner-clock-time-v1") {
    return privateJson({ ok: false, error: "Explicit Clock timing intent is required." }, 409);
  }
  ...
  try {
    ...
    return privateJson({ ok: false, error: "Clock timing body must be valid JSON." }, 400);
  }

  const date = body.date;
  const taskId = body.taskId;
  const localTime = body.localTime === "" ? null : body.localTime;
  ...
  if (!validLocalTime(localTime)) return privateJson({ ok: false, error: "localTime must be HH:MM or null." }, 400);
  ...
  const { data, error } = await supabase.rpc("owner_set_worker_day_task_time_api_v1", {
    ...
    const status = error.code === "42501" ? 403
      : error.code === "22023" ? 400
        : error.code === "55000" ? 409
          : 500;
    console.error("Atlas Clock timing update failed:", error);
    return privateJson({ ok: false, error: status === 500 ? "Atlas could not update this Clock placement." : error.message }, status);
  }
