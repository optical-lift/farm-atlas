...
function validWeekdays(value: unknown): value is number[] {
  ...
  return normalized.every((weekday) => Number.isInteger(weekday) && weekday >= 0 && weekday <= 6)
...
function privateJson(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "X-Atlas-Mutation": "owner-worker-day-shape-v1",
    },
  });
}
...
export async function POST(request: NextRequest) {
  ...
  if (request.headers.get("x-atlas-intent") !== "owner-worker-day-shape-v1") {
    return privateJson({ ok: false, error: "Explicit Worker Day shape intent is required." }, 409);
  }
  ...
  try {
    ...
    return privateJson({ ok: false, error: "Worker Day shape body must be valid JSON." }, 400);
  }

  const date = body.date;
  const weekdays = body.weekdays;
  const localStart = body.localStart;
  const localEnd = body.localEnd;
  ...
  const { data, error } = await supabase.rpc("owner_set_worker_day_shape_api_v1", {
    ...
    p_reason: "Owner-authored from Atlas Clock",
  });

  if (error) {
    const status = error.code === "42501" ? 403 : error.code === "22023" ? 400 : 500;
    console.error("Atlas Worker Day shape update failed:", error);
    return privateJson({ ok: false, error: status === 500 ? "Atlas could not update the Worker Day shape." : error.message }, status);
  }
