...
export async function POST(request: NextRequest) {
  ...
  const farmHandHarvestTab = authorized.access.membership.role === "farm_hand" && !operatorContext?.isOperating;
  ...
  if (action === "harvest") {
    ...
    const result = farmHandHarvestTab
      ? await supabase.rpc("record_flower_harvest_farm_hand_tab_v1", { p_farm_id: farmId, p_rows: normalizedRows, p_note: note, p_idempotency_key: idempotencyKey })
    ...
    return privateJson({ ...(result.data as Record<string, unknown>), ok: true, operatorMode: operatorContext?.isOperating ?? false, farmHandHarvestTab, effectiveMembershipId: operatorMembershipId });
  }
  ...
  if (action === "prepare") {
    ...
    const result = farmHandHarvestTab
      ? await supabase.rpc("record_flower_preparation_farm_hand_tab_v1", { p_farm_id: farmId, p_harvest_batch_id: harvestBatchId, p_outputs: normalizedOutputs, p_note: note, p_idempotency_key: idempotencyKey })
      : operatorMembershipId
