...
function privateJson(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "X-Atlas-Write-Path": "personal-atlas-portfolio-office-authoring-v1",
    },
  });
}
...
const authoringRoutes: Record<AuthoringKind, AuthoringRoute> = {
  owner_obligation: { normalize: ownerObligationInput },
  portfolio_thesis: { normalize: portfolioThesisInput },
  attention_policy: { normalize: attentionPolicyInput },
  operating_function: { normalize: operatingFunctionInput },
  great_game_scorecard: { normalize: greatGameScorecardInput },
  capital_request: { normalize: capitalRequestInput },
  investment_opportunity: { normalize: investmentOpportunityInput },
};
...
export async function POST(request: Request) {
  ...
  if (!kind || !route || !input || typeof input !== "object" || Array.isArray(input)) {
    return atlasApiError(400, "invalid_authoring_input", "A supported Portfolio Office authoring kind and input object are required.");
  }

  try {
    const supabase = await createAtlasServerClient();
    const normalizedInput = route.normalize(input as Record<string, unknown>);
    const { data, error } = await supabase.rpc("personal_portfolio_office_author_self_api_v1", {
      p_kind: kind,
    ...
    if (rpcError.code === "22023" || rpcError.code === "P0002" || error instanceof Error) {
      return atlasApiError(400, "portfolio_office_authoring_rejected", error instanceof Error ? error.message : rpcError.message ?? "Portfolio Office authoring was rejected.");
    }
    console.error("Atlas Personal Atlas Portfolio Office authoring failed:", error);
    return atlasApiError(500, "portfolio_office_authoring_failed", "Atlas could not save this Portfolio Office projection.");
  }
}
