      ...
      <section className="atlas-phone atlas-dashboard-phone atlas-task-page-phone atlas-overview-page-phone">
        ...
        <div className="atlas-task-page-body atlas-overview-body atlas-work-collection-body">
          <section className="atlas-overview-hero atlas-work-collection-hero">
            <div><strong>{typeFilter ? filtered[0]?.collection_label ?? "Maintenance" : "Unified Maintenance Plan"}</strong><span>{prettyDate(date)}</span></div>
            <p>{loading ? "Building plan" : `${filtered.length} objects · ${totalMinutes(filtered)} minutes`}</p>
          </section>

          <section className="atlas-overview-stat-grid" aria-label="Maintenance plan stats">
            <article><strong>{loading ? "…" : groups.length}</strong><span>collections</span></article>
            <article><strong>{loading ? "…" : totalMinutes(morning)}</strong><span>morning min</span></article>
            <article><strong>{loading ? "…" : totalMinutes(evening)}</strong><span>evening min</span></article>
            <article><strong>{loading ? "…" : filtered.filter((item) => item.must_precede_task).length}</strong><span>unlock work</span></article>
          </section>

          <section className="atlas-overview-summary-line"><p>Atlas derives these collections from canonical farm objects. Weather rules, equipment, cadence, condition, dependencies, and labor windows travel with each maintenance type.</p></section>
          ...
          {!loading && !error ? <section className="atlas-overview-zone-list atlas-work-collection-list">
            {groups.map(([type, collectionItems]) => (
              <section key={type} className="atlas-overview-zone-card atlas-work-collection-section">
                ...
                <div className="atlas-overview-task-list">
                  ...
                  <Link className="atlas-day-task-card atlas-work-collection-day-card" href={collectionItems[0].collection_path}><strong>Open {collectionItems[0].collection_label}</strong><span>Collection controls and completion</span><em>View this maintenance type</em></Link>
                </div>
              </section>
            ))}
