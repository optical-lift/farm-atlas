import Link from "next/link";

import AtlasOfflineRecovery from "@/components/atlas/pwa/AtlasOfflineRecovery";

export default function AtlasOfflinePage() {
  return (
    <main className="atlas-phone-shell atlas-home-shell atlas-task-page-shell">
      <section className="atlas-phone atlas-dashboard-phone atlas-task-page-phone">
        <header className="atlas-phone-top atlas-dashboard-top">
          <Link href="/" className="atlas-phone-brand atlas-task-header-brand">
            <span className="atlas-phone-kicker">Atlas</span>
            <span className="atlas-phone-title">Offline</span>
          </Link>
          <span className="atlas-weather-line">Waiting for field signal</span>
          <span className="atlas-note-plus" aria-hidden="true">○</span>
        </header>

        <div className="atlas-pwa-offline-body">
          <section>
            <span>Signal dropped</span>
            <h1>Atlas is still here.</h1>
            <p>Atlas will return to the view you were opening as soon as it can reach the farm again. Home, Work, Zone Registry, or Project views remain the recovery destinations.</p>
            <AtlasOfflineRecovery />
            <div>
              <Link href="/">Home</Link>
              <Link href="/day">Work</Link>
            </div>
          </section>
        </div>
      </section>
    </main>
  );
}
