...
const CHANNELS = [
  ["wholesale", "Wholesale"], ["farm_pickup", "Farm pickup"], ["delivery", "Delivery"], ["market", "Market"], ["subscription", "Subscription"], ["event", "Event"], ["other", "Other"],
] as const;
...
export default function FlowerCommercialSection() {
  ...
  function addLine() { const id = `sale-line-${nextLineId.current}`; nextLineId.current += 1; setLines((current) => [...current, { id, readyLotId: "", quantity: "", unitPrice: "" }]); }
    ...
    try {
      ...
      const response = await fetch("/api/atlas/flower-commerce", {
        ...
        body: JSON.stringify({
          ...
          note: note.trim() || null,
          idempotencyKey: `flower-sale:${farm.id}:${Date.now()}`,
        }),
      });
      const payload = await response.json() as { ok?: boolean; error?: string; saleOrderId?: string; fulfilled?: boolean };
  ...
  const validLines = farm && lines.length > 0 && lines.every((line) => {
  ...
  return (
    <>
      <AtlasCard as="section" className="atlas-commerce" ariaLabelledBy="atlas-available-title">
        ...
        {(data?.farms ?? []).filter((candidate) => candidate.available.length).map((candidate) => (
          <section className="atlas-commerce__farm" key={candidate.id}><header><small>Available at</small><h3>{candidate.name}</h3></header><div className="atlas-commerce__list">
            {candidate.available.map((lot) => <article className="atlas-commerce-item" key={lot.id}><div><small>Ready {prettyDate(lot.readyDate)}</small><h4>{KIND_LABELS[lot.inventoryKind] ?? lot.inventoryKind}</h4><p>{amount(lot.committedQuantity, lot.unit)} already committed from {amount(lot.birthQuantity, lot.unit, lot.quantityExactness === "lower_bound")}</p></div><strong>{amount(lot.availableQuantity, lot.unit, lot.quantityExactness === "lower_bound")}</strong></article>)}
          </div></section>
        ))}
      </AtlasCard>
      ...
      <AtlasCard as="section" className="atlas-commerce" ariaLabelledBy="atlas-sale-title">
        ...
        {!farm || !farm.available.length ? <div className="atlas-commerce__empty"><b>No Available inventory can be sold from this surface right now.</b><span>Ready inventory must exist and remain unclaimed before Atlas can create a commercial commitment.</span></div> : (
          <div className="atlas-commerce-form">
            {(data?.farms?.length ?? 0) > 1 ? <label><span>Farm</span><select value={farm.id} onChange={(event) => { setFarmId(event.target.value); setLines([{ id: `sale-line-${nextLineId.current++}`, readyLotId: "", quantity: "", unitPrice: "" }]); }} >{data?.farms?.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select></label> : null}
            <div className="atlas-commerce-form__grid">
              <label><span>Known buyer (optional)</span><select value={buyerId} onChange={(event) => setBuyerId(event.target.value)}><option value="">Walk-up / other</option>{farm.buyers.map((buyer) => <option key={buyer.id} value={buyer.id}>{buyer.businessName}</option>)}</select></label>
              <label><span>Customer label (optional)</span><input value={customerLabel} onChange={(event) => setCustomerLabel(event.target.value)} placeholder="Name for this order" /></label>
              <label><span>Sales channel</span><select value={salesChannel} onChange={(event) => setSalesChannel(event.target.value)}>{CHANNELS.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
              <label><span>Event / sale key (optional)</span><input value={eventKey} onChange={(event) => setEventKey(event.target.value)} placeholder="lobelia_farmers_market_2026" /></label>
            </div>

            <div className="atlas-commerce-lines">
              {lines.map((line, index) => {
                const lot = farm.available.find((candidate) => candidate.id === line.readyLotId);
                return <div className="atlas-commerce-line" key={line.id}>
              ...
              <button type="button" className="atlas-commerce__secondary" onClick={addLine}>+ Add another Ready lot</button>
            </div>

            <div className="atlas-commerce-form__grid">
              <label><span>Tax</span><input type="number" min="0" step="0.01" value={tax} onChange={(event) => setTax(event.target.value)} /></label>
              <label><span>Tip</span><input type="number" min="0" step="0.01" value={tip} onChange={(event) => setTip(event.target.value)} /></label>
              <label><span>Handoff</span><select value={fulfillmentMode} onChange={(event) => setFulfillmentMode(event.target.value)}><option value="immediate_handoff">Already handed off now</option><option value="pickup">Pickup later</option><option value="delivery">Delivery later</option></select></label>
            ...
            {fulfillmentMode !== "immediate_handoff" ? <div className="atlas-commerce-form__grid"><label><span>Due date</span><input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></label><label><span>Due time (optional)</span><input type="time" value={dueTime} onChange={(event) => setDueTime(event.target.value)} /></label><label><span>Fulfillment worker</span><select value={fulfillmentMembershipId} onChange={(event) => setFulfillmentMembershipId(event.target.value)}>{farm.members.map((member) => <option key={member.id} value={member.id}>{member.displayName} · {member.role.replace("_", " ")}</option>)}</select></label></div> : null}
            <label><span>Sale / handoff note (optional)</span><textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} /></label>
