...
import styles from "./farm-hand-harvest.module.css";
...
function errorMessage(error: FarmHandHarvestContext["error"], fallback: string) {
...
export default function FarmHandHarvestSection() {
  const [context, setContext] = useState<FarmHandHarvestContext | null>(null);
  ...
  const [harvestRows, setHarvestRows] = useState<HarvestDraft[]>([
    { id: "farm-hand-harvest-1", cropCycleId: "", bucketHalves: 0, moreAvailability: "yes" },
  ]);
  ...
  const [prepRows, setPrepRows] = useState<PrepDraft[]>([
    { id: "farm-hand-prep-1", productLabel: "", kind: "bundle", quantity: "1", stemsPerUnit: "5" },
  ]);
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch("/api/atlas/farm-hand-harvest", { cache: "no-store" });
      const payload = await response.json() as FarmHandHarvestContext;
      if (!response.ok || !payload.ok) throw new Error(errorMessage(payload.error, "Harvest could not be loaded."));
      setContext(payload);
      setHarvestBatchId((current) => current || payload.harvestBatches?.[0]?.id || "");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Harvest could not be loaded.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function submitHarvest() {
    ...
    const key = harvestKey.current ?? `farm-hand-harvest-tab:${crypto.randomUUID()}`;
    ...
    try {
      ...
      setHarvestRows([{ id: nextId("farm-hand-harvest"), cropCycleId: "", bucketHalves: 0, moreAvailability: "yes" }]);
      setHarvestNote("");
      harvestKey.current = null;
      setMessage("Harvest logged. It is in the same flower history as scheduled Harvest tasks.");
      await load();
    } catch (cause) {
  ...
  async function submitPreparation() {
    ...
    const key = prepKey.current ?? `farm-hand-prep-tab:${crypto.randomUUID()}`;
    prepKey.current = key;
    try {
      setSaving("prepare");
      setMessage(null);
      ...
      setPrepRows([{ id: nextId("farm-hand-prep"), productLabel: "", kind: "bundle", quantity: "1", stemsPerUnit: "5" }]);
      setPrepNote("");
      prepKey.current = null;
      setMessage("Prep batch logged. Those finished flowers are now in the farm's Ready inventory.");
      await load();
    } catch (cause) {
  ...
  if (error && !context) {
    return (
      <section className={styles.flowerLog} data-farm-hand-harvest="physical-work-only">
        <div className={styles.flowerLogHeader}><div><h2>Flower work</h2><p>Harvest and preparation logging for the work in your hands.</p></div></div>
  ...
  return (
    <section className={styles.flowerLog} data-farm-hand-harvest="physical-work-only">
      <div className={styles.flowerLogHeader}>
        <div>
          <h2>Flower work</h2>
          <p>These cards stay here so you can record another real harvest or prep batch whenever it happens. If Atlas already gave you a task for the work, finish that task card instead.</p>
        </div>
      ...
      <div className={styles.flowerQuickActions}>
        <details className={styles.flowerQuickCard}>
          <summary><span>Harvest Stems</span><small>Log another cut anytime</small></summary>
          <div className={styles.flowerForm}>
            ...
            <button className={styles.flowerAddButton} type="button" onClick={() => setHarvestRows((current) => [...current, { id: nextId("farm-hand-harvest"), cropCycleId: "", bucketHalves: 0, moreAvailability: "yes" }])}>+ Add another crop</button>
        ...
        <details className={styles.flowerQuickCard}>
          <summary><span>Condition + Bunch</span><small>Log another prep batch anytime</small></summary>
          <div className={styles.flowerForm}>
            <label>
            ...
            <button className={styles.flowerAddButton} type="button" onClick={() => setPrepRows((current) => [...current, { id: nextId("farm-hand-prep"), productLabel: "", kind: "bundle", quantity: "1", stemsPerUnit: "5" }])}>+ Add another finished line</button>
