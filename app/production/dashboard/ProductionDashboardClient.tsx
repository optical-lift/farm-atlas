...
export type ProductionDashboardPlan = {
  id: string;
  season_year: number;
  plan_label: string;
...
function cropName(plan: ProductionDashboardPlan) {
  return plan.crop_profiles?.crop_label || plan.plan_label.replace(/\s+\d{4}\s+production plan$/i, "");
}

function harvestGaps(plan: ProductionDashboardPlan) {
...
export default function ProductionDashboardClient({ initialPlans }: { initialPlans: ProductionDashboardPlan[] }) {
  ...
  return (
    <main className="atlas-season-shell">
      ...
      {!orderedPlans.length ? <div className="atlas-season-message">No active crop plans.</div> : null}
        ...
        return (
          <section className="atlas-season-plan" key={plan.id}>
            ...
            <div className="atlas-season-insights">
              <article><span>Harvest continuity</span><strong>{gaps.length ? `${gaps.length} projected gap${gaps.length === 1 ? "" : "s"}` : "Continuous projected harvest"}</strong>{gaps.length ? gaps.map((gap) => <p key={`${gap.after}-${gap.before}`}>{gap.days} days between S{gap.after} and S{gap.before}</p>) : <p>No positive gap between projected harvest windows.</p>}</article>
              <article><span>Frost boundary</span><strong>{pretty(plan.final_biological_sow_date)}</strong><p>{plan.protect_final_succession ? "Atlas prioritizes keeping the final viable succession intact." : "Atlas preserves the planned sequence even when earlier windows move."}</p></article>
            </div>
