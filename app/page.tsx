...
function taskAction(unit: TaskUnit | null) {
  if (!unit) return "Clear";
  const kind = captureKind(unit.card);
  if (kind === "germination") return "Check germination";
  if (kind === "weed") return "Weed bed";
  if (kind === "harvest") return "Cut stems";
  if (kind === "sow") return "Sow crop";
  if (kind === "bed_audit") return "Confirm bed contents";
  return cleanLabel(unit.card.title);
}

function taskFields(unit: TaskUnit | null) {
  if (!unit) return [];
  const kind = captureKind(unit.card);
  if (kind === "germination") return ["stand", "count", "gaps", "next"];
  if (kind === "weed") return ["finished", "pressure", "next"];
  if (kind === "harvest") return ["stems", "quality", "destination"];
  if (kind === "sow") return ["seed", "date", "spot"];
  if (kind === "bed_audit") return ["contents", "state", "next"];
  return ["note", "next"];
}
...
function CloseoutRecordCard({ record }: { record: CloseoutCardRecord }) {
  ...
  return (
    <article className="atlas-record-card">
      <div className="atlas-record-card-top"><span className="atlas-record-zone">{zone}</span><span className="atlas-record-kind">{kind}</span></div>
      <strong>{title}</strong>
      {crop ? <p className="atlas-record-crop">{crop}</p> : null}
...
function TaskController({ primaryUnit, nextUnits, cards, loading, openUnit, openTasks }: { primaryUnit: TaskUnit | null; nextUnits: TaskUnit[]; cards: AtlasTaskCard[]; loading: boolean; openTasks: () => void; openUnit: (unit: TaskUnit) => void }) {
  ...
  const fields = taskFields(primaryUnit);
  const date = primaryUnit?.card.due_date ? prettyDate(primaryUnit.card.due_date) : "Today";

  if (!primaryUnit) {
    return (
      <article className="atlas-home-box atlas-home-box-purple atlas-home-task-hero atlas-task-controller empty">
        <div className="atlas-task-controller-head"><span className="atlas-task-kicker">Today</span><span className="atlas-task-date">{date}</span></div>
        <button type="button" className="atlas-task-active-card" onClick={openTasks}><strong>{loading ? "Loading" : "Clear"}</strong><em>No open task selected.</em></button>
      </article>
    );
  }
  ...
  return (
    <article className="atlas-home-box atlas-home-box-purple atlas-home-task-hero atlas-task-controller">
      <div className="atlas-task-controller-head">
        <div><span className="atlas-task-kicker">Today</span><strong>{taskAction(primaryUnit)}</strong></div>
        <span className="atlas-task-date">{date}</span>
      </div>

      <div className="atlas-task-preview-grid">
        {nextUnits.slice(0, 2).map((unit) => (
          <button type="button" key={unit.id} className="atlas-task-queue-card" onClick={() => openUnit(unit)}>
            <strong>{taskAction(unit)}</strong>
            <span>{queueSubtitle(unit)}</span>
          </button>
        ))}
      </div>

      <button type="button" className="atlas-task-active-card" onClick={() => openUnit(primaryUnit)}>
        <span className="atlas-task-active-action">{taskAction(primaryUnit)}</span>
        <strong>{cropLine(primaryUnit)}</strong>
        <div className="atlas-task-chip-row">{fields.map((field) => <span key={field}>{field}</span>)}</div>
        <em>{locationLine(primaryUnit)}</em>
        <span className="atlas-task-record-button">Record</span>
      </button>

      <div className="atlas-task-count-row">
        <span><small>Open</small>{openCount}</span>
        <span><small>Done</small>{doneCount}</span>
        <span><small>Block</small>{blockedCount}</span>
      </div>
    </article>
  );
}
...
export default function AtlasHomePage() {
  ...
  function openUnit(unit: TaskUnit) {
    const kind = captureKind(unit.card);
    setOpenPanel(null); setSelectedUnit(unit); setCapture(defaultCapture(kind)); setResultNote(""); setResultMessage(null);
  }

  async function saveUnit(result: AtlasTaskResult, useCapture = true) {
    if (!selectedUnit) return;
    const kind = captureKind(selectedUnit.card);
    if (useCapture && kind === "germination" && !capture.standQuality) { setResultMessage("Stand required."); return; }
    try {
      setSavingResult(result); setResultMessage(null);
      ...
      await loadCards(); await loadRegistry(); await loadCloseout();
      setSelectedUnit(null); setResultNote("");
    } catch (saveError) { setResultMessage(saveError instanceof Error ? saveError.message : "Save failed."); }
  ...
  async function submitCloseout() {
    ...
    try {
      setCloseoutSaving(true); setCloseoutMessage(null);
      ...
      setCloseoutNote(""); setCloseoutCarry(""); setCloseoutNext(""); setCloseoutMessage("Closeout saved."); await loadCloseout();
    ...
    <main className="atlas-phone-shell atlas-home-shell">
      <section className="atlas-phone atlas-dashboard-phone">
        ...
        <div className="atlas-home-grid">
          <TaskController primaryUnit={primaryUnit} nextUnits={nextUnits} cards={cards} loading={loading} openUnit={openUnit} openTasks={() => setOpenPanel("tasks")} />
      ...
      {selectedUnit ? <section className="atlas-task-focus-overlay" role="dialog" aria-modal="true"><div className="atlas-task-focus-phone"><div className="atlas-task-focus-topbar"><div><strong>{taskAction(selectedUnit)}</strong>{cropLine(selectedUnit) ? <span>{cropLine(selectedUnit)}</span> : null}</div><button type="button" onClick={() => setSelectedUnit(null)}>Close</button></div><div className="atlas-task-focus-body"><section className="atlas-task-focus-purple atlas-work-card"><div className="atlas-task-focus-kicker"><span>{prettyDate(selectedUnit.card.due_date)}</span></div><h2>{taskAction(selectedUnit)}</h2>{sizeLine(selectedUnit.registryObject) ? <p>{sizeLine(selectedUnit.registryObject)} · {locationLine(selectedUnit)}</p> : <p>{locationLine(selectedUnit)}</p>}</section>
