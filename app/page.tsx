...
function purposeLine(unit: TaskUnit | null) {
  if (!unit) return "";
  const kind = captureKind(unit.card);
  if (kind === "germination") return "Record stand, gaps, and next action.";
  if (kind === "weed") return "Record pressure, finish, and next pass.";
  if (kind === "harvest") return "Record stems, quality, and destination.";
  if (kind === "sow") return "Record seed, date, and spot.";
  if (kind === "bed_audit") return "Confirm contents, state, and next step.";
  return "Add the field note and next step.";
}
  ...
  return (
    <article className="atlas-home-box atlas-home-box-purple atlas-home-task-hero atlas-task-controller">
      ...
      <div className="atlas-task-top-grid">
        {topCards.map((unit, index) => (
          <button type="button" key={unit.id} className="atlas-task-queue-card atlas-task-top-card" onClick={() => openUnit(unit)}>
            <small>{index === 0 ? "Next" : "Watch"}</small>
      ...
      <button type="button" className="atlas-task-active-card" onClick={() => openUnit(primaryUnit)}>
        ...
        <p className="atlas-task-purpose">{purposeLine(primaryUnit)}</p>
      </button>
      <div className="atlas-task-status-footer">
        <span><small>Today queue</small><strong>{visibleUnits.length} tasks</strong></span>
        <span><small>Needs data</small><strong>{needsDataCount} beds</strong></span>
        <span><small>Next pass</small><strong>{cleanLabel(nextPass)}</strong></span>
      </div>
    </article>
  );
}
    ...
    <main className="atlas-phone-shell atlas-home-shell">
      <section className="atlas-phone atlas-dashboard-phone">
        ...
        <div className="atlas-home-grid">
          <TaskController primaryUnit={primaryUnit} nextUnits={nextUnits} loading={loading} openUnit={openUnit} openTasks={() => setOpenPanel("tasks")} />
