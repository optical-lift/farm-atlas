...
function laneForTask(task: AtlasTaskCard): LaneKey {
  ...
  if (oldSurveyText(text) || text.includes("mow")) return "maintain";
...
function workWindowForTask(task: AtlasTaskCard, weatherLabel: string) { const text = taskText(task); const lane = laneForTask(task); const weather = weatherLabel.toLowerCase(); if (text.includes("mow")) return "Anytime"; if (weather.includes("rain") && (lane === "verify" || text.includes("germin") || text.includes("check") || text.includes("confirm"))) return "After rain"; if (text.includes("heat") || text.includes("water") || text.includes(ww) || text.includes("hoe") || lane === "harvest") return "Morning"; if (lane === "venue") return "Afternoon"; if (lane === "start" && (text.includes("grow room") || text.includes("tray") || text.includes("seed"))) return "Anytime"; if (lane === "verify") return "Morning"; return "Anytime"; }
...
function DetailCard({ lines }: { lines: string[] }) { if (!lines.length) return null; return <section className="atlas-task-detail-card"><strong>Details</strong>{lines.map((line) => <p key={line}>{line}</p>)}</section>; }
