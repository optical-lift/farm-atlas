...
async function pullProjectItem(formData: FormData) {
  ...
  const { data, error } = await supabase.rpc("pull_project_item_to_today_v1", {
    ...
    p_day: serviceDate,
    p_note: "Chosen from Daily Hand project pool.",
  });
...
export default async function ProjectPullPage({
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <Link className={styles.back} href="/">← Today</Link>
        <p className={styles.eyebrow}>Today’s Daily Hand · Physical progress</p>
        <h1 className={styles.title}>Choose one finish card.</h1>
        <p className={styles.intro}>
          Pick the piece that feels workable today. Everything you do not choose stays in the Elm Finish + Renovation Pool with no overdue date and nothing to move forward tomorrow.
        </p>

        <div className={styles.budget}>
          <span className={styles.pill}>Up to {projectMinutes} min today</span>
          <span className={styles.pill}>{options.capacity.remainingRegularMinutes} min regular capacity left</span>
        ...
        {status.completeForToday ? (
          <div className={styles.empty}>
            Today’s Finish Project pull is already chosen. That card is now an ordinary task in today’s hand.
          </div>
            ...
            {fittingOptions.map((option) => (
              <article className={styles.card} key={option.projectItemId}>
                ...
                <form action={pullProjectItem}>
                  ...
                  <button className={styles.choose} type="submit">Take this one today</button>
                </form>
              </article>
            ))}
          </div>
          ...
          <div className={styles.empty}>
            Nothing in this project fits the capacity still available today. The project stays exactly where it is; there is nothing to reschedule.
          </div>
        )}

        {notFittingOptions.length && !status.completeForToday ? (
          <p className={styles.quiet}>
            Atlas kept {notFittingOptions.length} additional ready {notFittingOptions.length === 1 ? "card" : "cards"} out of the choices above because they do not fit the remaining body budget today.
          </p>
        ) : null}
      </div>
    </main>
