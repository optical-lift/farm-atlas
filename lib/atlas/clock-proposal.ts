...
function openTask(item: CommittedItem) {
...
function overlaps(start: number, end: number, reservation: Reservation) {
...
function reserve(block: AtlasClockProposalBlock, reservations: Reservation[]) {
  reservations.push({ start: block.startMinute, end: block.endMinute });
}
...
function anchorRange(anchorTaskId: string, committedByTask: Map<string, CommittedItem>, existingByTask: Map<string, { startMinute: number; endMinute: number }>, proposalByTask: Map<string, AtlasClockProposalBlock>) {
  const proposed = proposalByTask.get(anchorTaskId);
  if (proposed) return { startMinute: proposed.startMinute, endMinute: proposed.endMinute };
  const existing = existingByTask.get(anchorTaskId);
  if (existing) return existing;
  const anchor = committedByTask.get(anchorTaskId);
  if (!anchor?.plannedStartAt) return null;
  const startMinute = clockLocalMinuteOfDay(anchor.plannedStartAt);
...
export function buildAtlasClockProposal(items: CommittedItem[]): AtlasClockProposalPlan {
  ...
  for (const item of untimed) {
    ...
    let block: AtlasClockProposalBlock | null = null;
    ...
    if (mobility.constraintClass === "fixed") {
      ...
      if (start === null) {
        unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: "Fixed timing is recorded, but Atlas cannot resolve its local clock time." });
        continue;
      }
      ...
      if (start === null) {
        unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: "No open span currently fits inside the recorded execution window." });
        continue;
      }
      block = proposal(item, start, duration.minutes, duration.source, "window", "Fits inside the task's recorded execution window.", false);
    } else {
      const bounds = dayWindowBounds[item.dayWindow];
      const start = firstFree(bounds.start, bounds.end, duration.minutes, reservations);
      if (start === null) {
        unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: `No open ${item.dayWindow} span currently fits this work.` });
        continue;
      }
    ...
    blocks.push(block);
    reserve(block, reservations);
    if (item.taskId) proposalByTask.set(item.taskId, block);
  }

  for (const item of untimed.filter((candidate) => candidate.mobility.constraintClass === "anchored")) {
    const duration = durationFor(item);
    const mobility = item.mobility;
    if (!mobility.anchorTaskId || !mobility.anchorRelation) {
      unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: "Atlas knows this work is anchored, but the recorded anchor is incomplete." });
      continue;
    }
    const anchor = anchorRange(mobility.anchorTaskId, committedByTask, existingByTask, proposalByTask);
    if (!anchor) {
      unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: "Its recorded anchor has no Clock position yet, so Atlas is leaving this work unplaced." });
      continue;
    }
    ...
    if (start === null) {
      unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: `No open span fits ${mobility.anchorRelation} its recorded anchor without breaking another Clock commitment.` });
      continue;
    }
