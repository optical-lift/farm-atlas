...
function ownerTaskResponsibility(task, ownerMembershipId) {
...
function taskBucket(task, today, weekEnd) {
  ...
  if (task.dueDate && task.dueDate > today && task.dueDate <= weekEnd) return "this_week";
  return "backlog";
}
...
function candidateBucket(candidate, today, weekEnd, timeZone) {
  ...
  if (dates.earliestTiming && dates.earliestTiming <= weekEnd) return { bucket: "this_week", dates };
...
export function buildOwnerMyWorkProjection({
  ...
  for (const task of tasks) {
    const responsibility = ownerTaskResponsibility(task, ownerMembershipId);
    if (!responsibility) {
      excludedTaskRows += 1;
      continue;
    }
    ...
    const bucket = taskBucket(task, today, weekEnd);
    const item = {
      key: `task:${task.id}`,
      source: "task",
