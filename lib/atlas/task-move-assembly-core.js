...
export function assembleTaskMoveCore({ task, execution, display, dominion, moveContext }) {
  ...
  const route = text(display?.route) || text(dominion?.route) || "general";
  const objects = (Array.isArray(task?.objects) ? task.objects : []).map(linkedObject).filter(Boolean);
  const actionLabel = text(display?.action) || text(execution?.doText) || text(dominion?.instruction) || title;
  const subjectLabel = text(display?.subject) || title;
  const workSiteLabel = text(execution?.placeText) || text(display?.location) || text(dominion?.placeLabel) || "Elm Farm";
