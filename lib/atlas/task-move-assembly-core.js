...
function inferredMoveRole(value) {
  const explicit = normalizeKey(value.move_role);
  if (explicit) return explicit;

  const category = normalizeKey(value.resource_category);
  if (["growing_medium", "water", "transport", "protective_equipment", "information"].includes(category)) {
    return category;
  }

  const type = normalizeKey(value.resource_type);
  if (["container", "tool", "equipment", "plant_material", "infrastructure"].includes(type)) return type;
  if (["soil_amendment", "consumable", "supply", "seed", "pest_control"].includes(type)) return "material";
  return "other";
}
...
function resourceRequirement(value) {
  ...
  const warningStatus = [
    ...
    || ["missing", "blocked", "needs_check"].includes(requirementStatus)
    || restockNeeded;

  return {
    id,
    requirementRole: text(value.requirement_role) || "required",
    moveRole: inferredMoveRole(value),
    label,
    ...
    requirementStatus: text(value.status) || "needed",
...
function expectedRequirements(task, actualResources) {
  ...
  const actualCategories = new Set(
    actualResources
      .flatMap((item) => [item.resourceCategory, item.resourceType, item.moveRole])
      .map(normalizeKey)
      .filter(Boolean),
  );
  const expected = [];
