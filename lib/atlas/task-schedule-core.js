...
function normalizeTask(row, startDate, endDate) {
  const source = asRecord(row);
  const status = text(source.status) ?? "open";
  const dueDate = text(source.due_date);
  const countsForWindow =
    booleanValue(source.counts_for_window) &&
    Boolean(dueDate) &&
    dueDate >= startDate &&
    dueDate <= endDate;
...
export function buildTaskScheduleProjection({ rows, startDate, endDate }) {
  ...
  const tasks = (Array.isArray(rows) ? rows : [])
    .map((row) => normalizeTask(row, startDate, endDate))
    .filter(
      (task) =>
        task.taskId &&
        (task.countsForWindow || ["blocked", "overdue", "undated"].includes(task.lane)),
    )
    .sort((left, right) => {
