export type AtlasWorkerDayShapeCommand = {
  serviceDate: string;
  weekdays: number[];
  localStart: string;
  localEnd: string;
...
export type AtlasWorkerDayShapeCommandResponse = {
  ok: true;
  date: string;
  result?: unknown;
};
...
export async function commitAtlasWorkerDayShape(command: AtlasWorkerDayShapeCommand) {
  const response = await fetch("/api/atlas/owner-worker-day-shape", {
    method: "POST",
    ...
    headers: {
      ...
      "x-atlas-intent": "owner-worker-day-shape-v1",
    },
    body: JSON.stringify({
      date: command.serviceDate,
      weekdays: command.weekdays,
      localStart: command.localStart,
      localEnd: command.localEnd,
    }),
  });
