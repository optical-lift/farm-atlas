...
export function atlasTaskIsOutdoor(task: AtlasTaskCard) {
  ...
  const haystack = [task.title, task.task_type, task.action_key, task.work_class, task.zone_key, task.zone_label, ...task.objects.map((object) => `${object.object_key} ${object.object_label} ${object.object_type}`)]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
export function atlasFarmHandConveyorMoves(home: AtlasUniversalHomeModel, date = new Date()) {
  if (atlasFarmHandOutsideWorkIsAvailable(date)) return home.moves;
  const available = home.moves.filter((move) => !atlasFarmHandMoveIsOutdoor(home, move));
  const hasHeldOutdoorWork = available.length < home.moves.length;
  if (available.length || !hasHeldOutdoorWork) return available;
  return [{
    key: "farm-hand:outside-window",
    kind: "attention" as const,
    category: "Heat window",
    title: "Outside work resumes at 7pm",
    scopeLabel: home.activeFarm?.farmName ?? "Elm Farm",
    meta: "Indoor work only for now",
    detail: "Atlas is holding field work until the evening outside-work window.",
    href: "/work/today",
