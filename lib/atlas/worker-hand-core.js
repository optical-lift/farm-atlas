...
function workerInstruction(row) {
  const metadata = asRecord(row.metadata);
  const executionHow = stringList(metadata.execution_how);
  if (executionHow.length) return executionHow.join(" · ");
  return text(metadata.display_detail) ?? text(row.instruction);
}
...
function task(value) {
  ...
  return {
    ...
    instruction: workerInstruction(row),
    ...
    commitmentKind: text(row.commitment_kind),
    effortUnits: numeric(row.effort_units),
    taskScope: text(row.task_scope),
  };
}
