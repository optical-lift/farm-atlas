...
export function normalizeAtlasSession({ user, profile, memberships, organizationMemberships }) {
  ...
  const activeOrganizationId = normalizedOrganizationMemberships.some(
  ...
  const onboardingState =
    asText(profileRow.onboarding_state) ??
    (normalizedMemberships.length > 0 || normalizedOrganizationMemberships.length > 0 ? "ready" : "new");
