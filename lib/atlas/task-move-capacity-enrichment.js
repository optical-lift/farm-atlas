...
function canonicalCapacityRequirement(row) {
  ...
  if (requirementStatus === "waived" || requirementStatus === "satisfied") {
    ...
    status = "blocked";
  } else if (capacityStatus !== "confirmed" || totalCapacity === null) {
    status = required ? "blocked" : "warning";
  } else if (required && quantity !== null && totalCapacity < quantity) {
    status = "blocked";
  } else {
    // A confirmed pool total is not the same thing as currently available capacity.
    // Until Pass 3 has a trustworthy occupancy/reservation answer, keep it visible as a warning.
    status = "warning";
  }
  ...
  return {
    ...
    capacityStatus,
    totalCapacity,
    totalUnit: text(row.pool_unit) || null,
    questions,
  };
}
