...
/** @param {{ title?: unknown, task_type?: unknown, metadata?: Record<string, unknown> | null } | null | undefined} task */
export function classifyAtlasTaskWorkflow(task) {
  const metadata = task?.metadata ?? {};
  if (
    task?.task_type === "grow_room_care"
    && text(task?.title).toLowerCase() === "grow room care"
    && (truthy(metadata.round_completion_required) || truthy(metadata.manual_top_level_card))
  ) return "grow_room_round";
