...
function unresolvedReason(item: AtlasWorkerDayChronologyItem) {
  if (item.chronologyState === "awaiting_day_shape") {
    return "Owner Day Shape is required before Atlas can propose a clock time.";
  }
  if (item.chronologyState === "blocked_policy_conflict") {
    return "More than one Worker Day policy applies here. Resolve the Day Shape conflict before planning this Clock.";
  }
  if (item.chronologyState === "unplaced_no_lawful_interval") {
    return "No open interval inside the authored Worker Day and this task's day window can hold the work.";
  }
  return "Atlas has not found a lawful proposal interval for this work.";
}

export function buildAtlasClockProposalFromChronology(
  items: CommittedItem[],
  chronology: AtlasWorkerDayChronology | null,
): AtlasClockProposalPlan {
  if (!chronology) return { blocks: [], unresolved: [] };
  const openByTaskId = new Map(
    items
      .filter((item) => item.taskId && item.status !== "done" && item.status !== "completed")
  ...
  for (const entry of chronology.items) {
    ...
    if (entry.chronologyState === "proposed" && entry.startsAt) {
      ...
      blocks.push({
        id: `server-clock-proposal:${item.id}`,
        taskId: item.taskId,
        item,
        startMinute,
        endMinute: startMinute + entry.durationMinutes,
        durationMinutes: entry.durationMinutes,
        durationSource: durationSource(item),
        placementSource: "day_window",
        reason: "Fits inside the owner-authored Worker Day Shape after real reservations and committed Clock placements are removed.",
        conflict: false,
      });
      continue;
    }

    if (["awaiting_day_shape", "blocked_policy_conflict", "unplaced_no_lawful_interval"].includes(entry.chronologyState)) {
      unresolved.push({ id: item.id, taskId: item.taskId, title: item.title, reason: unresolvedReason(entry) });
    }
  }

  return {
    blocks: blocks.sort((left, right) => left.startMinute - right.startMinute || left.item.sequenceOrder - right.item.sequenceOrder),
    unresolved,
  };
}
