...
export type WorkerWeekProjectionItem = {
  id: string;
  plannedDate: string;
...
export type WorkerWeekProjectionDay = {
  date: string;
  items: WorkerWeekProjectionItem[];
};

export type WorkerWeekProjection = {
  farmId: string;
  membershipId: string;
  startDate: string;
  endDate: string;
  paidTargetMinutes: number;
  days: WorkerWeekProjectionDay[];
};
...
async function readProjection(
  ...
  dayCount: number,
  refresh: boolean,
): Promise<WorkerWeekProjection> {
  const safeDayCount = Math.max(1, Math.min(dayCount, 14));
  const endDate = addDays(startDate, safeDayCount - 1);

  if (refresh) {
    const refreshResult = await atlasSupabase.schema("atlas").rpc("refresh_worker_week_projection_v1", {
      p_farm_id: farmId,
  ...
  const rows = (projectionResult.data ?? []) as Array<{
    id: string;
    planned_date: string;
    source_kind: WorkerWeekProjectionItem["sourceKind"];
    source_id: string;
    title: string;
    plan_state: WorkerWeekProjectionItem["planState"];
    environment: string | null;
  ...
  const days: WorkerWeekProjectionDay[] = Array.from({ length: safeDayCount }, (_, index) => {
...
export async function readWorkerWeekProjection(
  farmId: string,
  membershipId: string,
  startDate: string,
  dayCount = 7,
...
/**
 * Read the already-built Worker planning ledger without recomputing it. Future
 * Day views use this path so looking ahead cannot mutate or erase tentative work.
 */
export async function readStoredWorkerWeekProjection(
  farmId: string,
  membershipId: string,
  startDate: string,
  dayCount = 7,
