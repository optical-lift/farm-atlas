...
begin
  ...
  -- A future fixed recurrence is already real calendar truth even when its task row
  -- is intentionally unreleased. Project that occurrence read-only into the Day
  -- instead of manufacturing a task early. Once release creates the task,
  -- released_task_id stops this preview row from being emitted.
  if p_day > v_today then
    select coalesce(
      jsonb_agg(
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'reason', 'Planned recurring occurrence; task releases through its governed recurrence.'
        ))
        order by
    ...
    where occurrence.farm_id = p_farm_id
      ...
      and occurrence.source_kind = 'recurring_task'
