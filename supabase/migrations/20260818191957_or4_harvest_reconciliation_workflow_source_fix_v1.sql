...
declare
  ...
  v_prod_key text;
begin
  select * into v_event from atlas.crop_harvest_events where id=p_crop_harvest_event_id;
  if v_event.id is null then raise exception 'Crop harvest event not found.' using errcode='P0002'; end if;
  ...
  select count(distinct link.production_lot_id)::integer,min(link.production_lot_id::text)::uuid
  into v_lot_count,v_lot_id
  from atlas.production_lot_crop_cycles link
  where link.crop_cycle_id=v_event.crop_cycle_id and link.confidence='confirmed';
  ...
  if v_lot_count<>1 then
    ...
    values (
      ...
      'crop_cycle',v_event.crop_cycle_id,v_event.id::text,'production_reconciliation_required',v_event.observed_date,
      jsonb_build_object(
        'contractVersion','or4_flower_harvest_result_state_transition_v1','reason',v_reason,
        'confirmedProductionLotCount',v_lot_count,'cropHarvestEventId',v_event.id,'cropCycleId',v_event.crop_cycle_id,'taskId',v_event.task_id,
  ...
  begin v_quantity := nullif(v_event.metadata->>'bucketEquivalentFloor','')::numeric;
  ...
  if v_quantity is null then
    ...
    values (
      ...
      jsonb_build_object(
        'contractVersion','or4_flower_harvest_result_state_transition_v1','reason',v_reason,
        'productionLotId',v_lot_id,'cropHarvestEventId',v_event.id,'cropCycleId',v_event.crop_cycle_id,
  ...
  select id into v_prod_event_id from atlas.production_lot_events where farm_id=v_event.farm_id and idempotency_key=v_prod_key;
  if v_prod_event_id is null then
    insert into atlas.production_lot_events(
      farm_id,production_lot_id,event_type,event_date,quantity,unit,task_id,crop_cycle_id,object_id,note,source,idempotency_key,metadata
    ) values (
      v_event.farm_id,v_lot_id,'harvest_recorded',v_event.observed_date,v_quantity,'bucket_equivalent',v_event.task_id,v_event.crop_cycle_id,v_object_id,
      v_event.note,'flower_harvest_or4',v_prod_key,
      jsonb_build_object(
        'harvestEvidenceKind','flower_bucket_observation','quantityExactness',v_exactness,
        'bucketBand',v_event.metadata->>'bucketBand','bucketEquivalentFloor',v_quantity,'moreAvailability',v_more_availability,
        'sourceCropHarvestEventId',v_event.id,'sourceFlowerHarvestObservationId',v_event.metadata->>'flowerHarvestObservationId',
...
exception when others then
  begin
    ...
    values (
      v_event.farm_id,'or4:harvest-production-reconciliation:'||v_event.id::text,
      'crop_cycle',v_event.crop_cycle_id,v_event.id::text,'production_reconciliation_required',coalesce(v_event.observed_date,(now() at time zone 'America/Chicago')::date),
      jsonb_build_object(
        'contractVersion','or4_flower_harvest_result_state_transition_v1','reason','production_bridge_failed','error',sqlerrm,
        'cropHarvestEventId',v_event.id,'cropCycleId',v_event.crop_cycle_id,'taskId',v_event.task_id,
