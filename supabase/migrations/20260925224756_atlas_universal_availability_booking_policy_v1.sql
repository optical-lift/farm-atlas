...
CREATE OR REPLACE FUNCTION reality.subject_schedule_availability_v1(p_subject_kind text, p_subject_id uuid, p_starts_at timestamp with time zone, p_ends_at timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION ledger.evaluate_booking_policies_v1(p_ledger_id uuid, p_resource_ids uuid[], p_booking_kind text, p_starts_at timestamp with time zone, p_ends_at timestamp with time zone, p_is_recurring boolean DEFAULT false, p_as_of timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION ledger.resource_claim_availability_v1(p_ledger_id uuid, p_resource_id uuid, p_starts_at timestamp with time zone, p_ends_at timestamp with time zone, p_requested_claim_kind text DEFAULT 'exclusive'::text, p_requested_quantity numeric DEFAULT NULL::numeric, p_exclude_claim_id uuid DEFAULT NULL::uuid, p_exclude_occurrence_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION ledger.establish_booking_bundle_service_v1(p_ledger_id uuid, p_occurrence_id uuid, p_booking_kind text, p_claims jsonb, p_customer_entity_id uuid DEFAULT NULL::uuid, p_business_model_key text DEFAULT NULL::text, p_booking_label text DEFAULT NULL::text, p_booking_state text DEFAULT 'hold'::text, p_created_by_seat_id uuid DEFAULT NULL::uuid, p_metadata jsonb DEFAULT '{}'::jsonb, p_provenance jsonb DEFAULT '{}'::jsonb, p_idempotency_key text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION atlas.upsert_ledger_availability_profile_self_api_v1(p_ledger_id uuid, p_subject_kind text, p_subject_id uuid, p_timezone_name text, p_default_state text DEFAULT 'available'::text, p_profile_state text DEFAULT 'active'::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION atlas.upsert_ledger_availability_rule_self_api_v1(p_ledger_id uuid, p_profile_id uuid, p_stable_key text, p_rule_effect text, p_effective_start_date date, p_effective_end_date date DEFAULT NULL::date, p_weekdays smallint[] DEFAULT ARRAY[(0)::smallint, (1)::smallint, (2)::smallint, (3)::smallint, (4)::smallint, (5)::smallint, (6)::smallint], p_all_day boolean DEFAULT false, p_local_start_time time without time zone DEFAULT NULL::time without time zone, p_local_end_time time without time zone DEFAULT NULL::time without time zone, p_priority integer DEFAULT 0, p_rule_state text DEFAULT 'active'::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION atlas.upsert_ledger_availability_exception_self_api_v1(p_ledger_id uuid, p_profile_id uuid, p_stable_key text, p_local_date date, p_exception_effect text, p_all_day boolean DEFAULT true, p_local_start_time time without time zone DEFAULT NULL::time without time zone, p_local_end_time time without time zone DEFAULT NULL::time without time zone, p_priority integer DEFAULT 100, p_exception_state text DEFAULT 'active'::text, p_reason text DEFAULT NULL::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION atlas.upsert_ledger_booking_policy_self_api_v1(p_ledger_id uuid, p_stable_key text, p_policy_kind text, p_config jsonb, p_resource_id uuid DEFAULT NULL::uuid, p_booking_kind text DEFAULT NULL::text, p_priority integer DEFAULT 0, p_policy_state text DEFAULT 'active'::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
