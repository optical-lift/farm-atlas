...
begin
  ...
  select count(distinct tx.object_id), (array_agg(distinct tx.object_id))[1]
...
begin
  ...
  for r in
    select t.id as task_id, (array_agg(distinct tx.object_id))[1] as object_id
    from atlas.tasks t
