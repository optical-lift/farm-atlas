...
alter table atlas.planned_work_occurrences
  add constraint planned_work_occurrences_miss_consequence_object_check check (jsonb_typeof(miss_consequence)='object'),
  add constraint planned_work_occurrences_lawful_window_order_check check (earliest_lawful_date is null or latest_lawful_date is null or latest_lawful_date >= earliest_lawful_date),
  add constraint planned_work_occurrences_preferred_window_order_check check (preferred_start_date is null or preferred_end_date is null or preferred_end_date >= preferred_start_date),
  add constraint planned_work_occurrences_preferred_after_earliest_check check (earliest_lawful_date is null or ((preferred_start_date is null or preferred_start_date >= earliest_lawful_date) and (preferred_end_date is null or preferred_end_date >= earliest_lawful_date))),
  add constraint planned_work_occurrences_preferred_before_latest_check check (latest_lawful_date is null or ((preferred_start_date is null or preferred_start_date <= latest_lawful_date) and (preferred_end_date is null or preferred_end_date <= latest_lawful_date))),
  add constraint planned_work_occurrences_hard_finish_after_earliest_check check (earliest_lawful_date is null or hard_finish_date is null or hard_finish_date >= earliest_lawful_date),
  add constraint planned_work_occurrences_latest_before_hard_finish_check check (latest_lawful_date is null or hard_finish_date is null or latest_lawful_date <= hard_finish_date),
  add constraint planned_work_occurrences_preferred_before_hard_finish_check check (hard_finish_date is null or ((preferred_start_date is null or preferred_start_date <= hard_finish_date) and (preferred_end_date is null or preferred_end_date <= hard_finish_date)));

comment on column atlas.planned_work_occurrences.not_before_date is 'Release eligibility/horizon date. This is NOT the earliest lawful execution date. A long release horizon may make this years earlier than the work may actually be performed.';
comment on column atlas.planned_work_occurrences.planned_due_date is 'Planning/target date for the durable obligation. This is NOT automatically a latest-lawful date or hard finish deadline.';
comment on column atlas.planned_work_occurrences.earliest_lawful_date is 'Earliest date on which execution is lawful/biologically permissible when the upstream source actually defines such a bound. NULL means unknown/unbounded by this contract, not today.';
comment on column atlas.planned_work_occurrences.preferred_start_date is 'Start of the preferred execution window when upstream truth defines one. Preference does not itself make dates outside this window unlawful.';
comment on column atlas.planned_work_occurrences.preferred_end_date is 'End of the preferred execution window when upstream truth defines one. Preference does not itself make dates outside this window unlawful.';
comment on column atlas.planned_work_occurrences.latest_lawful_date is 'Latest date on which execution remains lawful/biologically valid when upstream truth defines one. NULL means unknown/unbounded by this contract.';
comment on column atlas.planned_work_occurrences.hard_finish_date is 'Hard completion boundary when upstream truth defines one. Never inferred merely from planned_due_date or tasks.due_date.';
comment on column atlas.planned_work_occurrences.miss_consequence is 'Structured upstream consequence if the lawful/preferred timing is missed. Empty object means consequence is not yet canonically known.';
comment on column atlas.planned_work_occurrences.temporal_contract_source is 'Canonical source/version that supplied the typed lawful-time contract. Required when the replacement helper writes any non-empty temporal contract.';
comment on column atlas.planned_work_occurrences.temporal_contract_updated_at is 'When the typed lawful-time contract was last replaced from an authoritative source.';
