-- Preserve and retire stale dependency claims that no longer have live downstream truth.
update atlas.tasks
set metadata=(metadata
      - 'completion_unlocks_sowing'
      - 'sowing_unlock_scope')
      || jsonb_build_object(
        'superseded_dependency_claim',jsonb_build_object(
          'completion_unlocks_sowing',coalesce(metadata->'completion_unlocks_sowing','false'::jsonb),
          'sowing_unlock_scope',metadata->>'sowing_unlock_scope',
          'superseded_at',now(),
          'reason','Entry Billboard fall-crop plan was cancelled on 2026-08-15 and the real downstream prerequisites were deactivated; the reset no longer unlocks those plantings.'
        ),
        'classification_correction_source','worker_consequence_structural_resolution_v2',
        'classification_correction_at',now()
      ),
    work_lane='discretionary',
    commitment_kind='floating',
    updated_at=now()
where id='a6817742-babf-426c-88fd-545695392016'
  and status='open';

update atlas.tasks
set metadata=(metadata
      - 'dependent_task_ids'
      - 'dependent_task_labels')
      || jsonb_build_object(
        'superseded_dependency_claim',jsonb_build_object(
          'dependent_task_ids',coalesce(metadata->'dependent_task_ids','[]'::jsonb),
          'dependent_task_labels',coalesce(metadata->'dependent_task_labels','[]'::jsonb),
          'superseded_at',now(),
          'reason','Legacy dependent task IDs no longer exist and no canonical task_prerequisites edge exists; do not infer production-prerequisite status from dead references.'
        ),
        'dependency_hint_reconciled_at',now(),
        'dependency_hint_reconciled_by','worker_consequence_structural_resolution_v2'
      ),
    updated_at=now()
where id='35b3fb24-70f9-4b7d-bf80-c2028561ab7c'
  and status='open';
...
begin
  ...
  if v_explicit<>'{}'::jsonb then
    ...
    v_basis:=v_basis||jsonb_build_array(jsonb_build_object('code','routine_production_without_known_deadline'));
     ...
     or (
       coalesce(v_task.work_lane,'')='discretionary'
       and coalesce(v_capacity.effective_obligation_class,'')<>'routine_production'
     )
     or (
