...
begin
  ...
  -- Service-internal helper: authenticated execution is revoked. Its public
  -- worker/Owner callers enforce membership authorization before invoking it.
  ...
  with target_presented as (
    select p.task_id from atlas.presented_work_rows_v1(p_farm_id,p_membership_id,p_work_date) p
