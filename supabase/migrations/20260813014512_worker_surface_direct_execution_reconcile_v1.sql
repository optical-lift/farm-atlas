...
select p.farm_id,p.organization_id,p.zone_id,'Sow ProCut Horizon — BW7','sowing_bed','open',p.priority,p.due_date,
  jsonb_build_object(
    'task_key','owner_20260808_sow_procut_horizon_bw7','group_source_task_id',p.id,'surface_source_kind','grouped_worker_move',
    'stateful_child',true,'worker_execution_child',true,'checklist_group_label','Beds','checklist_label','BW7','checklist_action_label','Mark sown',
    'display_action','Sow','display_subject','BW7','display_location',g.label,
    'planting_log_required',true,'planting_log_auto_capture',true,'planting_log_default_amount','1','planting_log_unit','bed',
    'planting_log_default_zone_id',p.zone_id::text,'planting_log_default_object_id',g.id::text,
    'planting_log_default_location',g.label,'planting_log_crop_label','Sunflower','planting_log_variety','ProCut Horizon',
    'planting_method','direct_sow','crop_profile_id',p.metadata->>'crop_profile_id','crop_profile_stable_key','sunflower_procut_horizon',
    'rows_per_3ft_bed',3,'in_row_spacing_in',4
  ),
  'sow','standard',p.id,'assigned_worker',p.assigned_membership_id,p.assigned_user_id,p.created_by_user_id,
  p.origin_kind,p.work_lane,p.commitment_kind,coalesce(p.operation_class,'establish_aboveground'),
...
select p.farm_id,p.organization_id,p.zone_id,'Sow ProCut Horizon — BW8','sowing_bed','open',p.priority,p.due_date,
  jsonb_build_object(
    'task_key','owner_20260808_sow_procut_horizon_bw8','group_source_task_id',p.id,'surface_source_kind','grouped_worker_move',
    'stateful_child',true,'worker_execution_child',true,'checklist_group_label','Beds','checklist_label','BW8','checklist_action_label','Mark sown',
...
with source as (
  ...
  where title='Hang conference-room café lights + porch solar lights'
    and assigned_membership_id is not null and metadata ? 'superseded_by_task_keys'
  order by updated_at desc limit 1
)
insert into atlas.tasks(
  farm_id,organization_id,zone_id,title,task_type,status,priority,due_date,metadata,action_key,work_class,
  visibility_scope,assigned_membership_id,assigned_user_id,created_by_user_id,origin_kind,work_lane,commitment_kind,
...
select s.farm_id,s.organization_id,s.zone_id,'Venue Lighting — Conference Room Café Lights','venue_lighting','open',s.priority,
  ...
  jsonb_build_object(
    'task_key','anna_20260812_venue_lighting_conference_room','source_task_id',s.id,'surface_source_kind','worker_surface_split',
    'display_family','Venue Lighting','operation_family','Venue Lighting','display_action','Hang','display_subject','Café lights',
    'display_location','Conference room','execution_place','Conference room','execution_do','Hang café lights from ceiling hooks.',
...
select s.farm_id,s.organization_id,s.zone_id,'Venue Lighting — Porch Solar Lights','venue_lighting','open',s.priority,
  coalesce((s.metadata#>>'{last_transition,target_date}')::date,date '2026-08-12'),
  jsonb_build_object(
    'task_key','anna_20260812_venue_lighting_porch_solar','source_task_id',s.id,'surface_source_kind','worker_surface_split',
    'display_family','Venue Lighting','operation_family','Venue Lighting','display_action','Place','display_subject','Solar lights',
    'display_location','Porches','execution_place','Porches','execution_do','Place solar lights outside and position them to charge.',
    'window_key','evening','work_order_anchor','evening'
  ),
...
with source as (
  select * from atlas.tasks where metadata->>'task_key'='anna_20260812_prep_bloom_bar_coffee_wrapping' order by updated_at desc limit 1
)
insert into atlas.tasks(
  farm_id,organization_id,zone_id,title,task_type,status,priority,due_date,metadata,action_key,work_class,
  visibility_scope,assigned_membership_id,assigned_user_id,created_by_user_id,origin_kind,work_lane,commitment_kind,
...
select s.farm_id,s.organization_id,s.zone_id,'Coffee Bar — Start Cold Brew','hospitality_prep','open',s.priority,date '2026-08-12',
  jsonb_build_object(
    'task_key','anna_20260812_coffee_bar_start_cold_brew','source_task_id',s.id,'surface_source_kind','worker_surface_split',
    'display_family','Coffee Bar','operation_family','Coffee Bar','display_action','Prepare','display_subject','Cold brew',
    'display_location','Coffee Bar','execution_place','Coffee Bar','execution_do','Start the cold brew for Thursday.',
    'operation_place_key','coffee_bar','place_readiness_on_done',true,'window_key','evening','work_order_anchor','evening'
  ),
...
select s.farm_id,s.organization_id,s.zone_id,'Wrapping Station — Stage Thursday Supplies','fulfillment_prep','open',s.priority,date '2026-08-12',
  jsonb_build_object(
    'task_key','anna_20260812_wrapping_station_stage_supplies','source_task_id',s.id,'surface_source_kind','worker_surface_split',
    'display_family','Wrapping Station','operation_family','Wrapping Station','display_action','Stage','display_subject','Thursday wrapping supplies',
    'display_location','Wrapping station','execution_place','Wrapping station','execution_do','Stage the wrapping station for Thursday.',
    'operation_place_key','bouquet_wrapping_station','place_readiness_on_done',true,'window_key','evening','work_order_anchor','evening'
  ),
...
insert into atlas.tasks(
  farm_id,organization_id,zone_id,title,task_type,status,priority,due_date,metadata,action_key,work_class,parent_task_id,
  visibility_scope,assigned_membership_id,assigned_user_id,created_by_user_id,origin_kind,work_lane,commitment_kind,
  operation_class_source,task_scope,created_at,updated_at
)
