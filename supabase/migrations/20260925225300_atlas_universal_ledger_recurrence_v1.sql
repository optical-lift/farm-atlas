-- Production migration: 20260925225300_atlas_universal_ledger_recurrence_v1
-- Ledger-native recurrence with local wall-clock semantics, exceptions, and occurrence materialization.
...
create table ledger.recurrence_series (
  ...
  check (
    ...
    or (frequency='weekly' and cardinality(weekdays)>0 and cardinality(month_ordinals)=0)
    or (frequency='monthly_nth_weekday' and cardinality(weekdays)>0 and cardinality(month_ordinals)>0)
  ),
  check (
    (all_day and local_start_time is null and local_end_time is null)
    or (not all_day and local_start_time is not null)
  ),
...
create table ledger.recurrence_exceptions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check ((exception_action='move' and replacement_date is not null)
