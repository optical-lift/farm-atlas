-- Reconcile the EB1–EB6 task in both canonical columns and mirrored metadata so
-- task-normalization triggers cannot restore the cancelled dependency classification.
update atlas.tasks
set work_lane='discretionary',
    ...
    metadata=(metadata
      ...
      || jsonb_build_object(
        ...
        'commitment_kind','floating',
        'classification_correction_source','worker_living_consequence_alignment_and_eb_reset_v3',
        'classification_correction_at',now(),
        'dependency_cancelled_reason','Entry Billboard fall-crop plan cancelled 2026-08-15; former spinach/lettuce prerequisite rows are inactive and crops moved to Main Garden.'
      ),
    updated_at=now()
