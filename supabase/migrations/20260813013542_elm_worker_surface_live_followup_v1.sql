...
select
  ...
  jsonb_build_object(
    'task_key','owner_20260808_sow_procut_horizon_bw7','stateful_child',true,'worker_execution_child',true,
    ...
    'planting_log_default_zone_id',parent.zone_id::text,'planting_log_default_object_id',go.id::text,'planting_log_default_location',go.label,
    'planting_log_crop_label','Sunflower','planting_log_variety','ProCut Horizon','planting_method','direct_sow',
    'crop_profile_id',parent.metadata->>'crop_profile_id','crop_profile_stable_key','sunflower_procut_horizon','rows_per_3ft_bed',3,'in_row_spacing_in',4
  ),
...
select
  ...
  'grouped_worker_move',parent.id,
  jsonb_build_object(
    'task_key','owner_20260808_sow_procut_horizon_bw8','stateful_child',true,'worker_execution_child',true,
...
select old.farm_id,old.organization_id,old.zone_id,
  ...
  'worker_surface_split',old.id,jsonb_build_object(
    'task_key','anna_20260812_venue_lighting_conference_room','display_family','Venue Lighting','operation_family','Venue Lighting','display_action','Hang',
  ...
  old.created_by_user_id,old.origin_kind,old.work_lane,old.commitment_kind,'host_prepare','worker_surface_grammar_v1','farm_operation',
  now(),'split_real_operation',now(),now()
...
select old.farm_id,old.organization_id,old.zone_id,
  ...
  coalesce((old.metadata#>>'{last_transition,target_date}')::date,(old.metadata->>'execution_date')::date,date '2026-08-12'),
  'worker_surface_split',old.id,jsonb_build_object(
    'task_key','anna_20260812_venue_lighting_porch_solar','display_family','Venue Lighting','operation_family','Venue Lighting','display_action','Place',
...
select old.farm_id,old.organization_id,old.zone_id,
  ...
  'worker_surface_split',old.id,jsonb_build_object(
    'task_key','anna_20260812_coffee_bar_start_cold_brew','display_family','Coffee Bar','operation_family','Coffee Bar','display_action','Prepare',
    'display_subject','Cold brew','display_location','Coffee Bar','execution_place','Coffee Bar','execution_do','Start the cold brew for Thursday.',
...
select old.farm_id,old.organization_id,old.zone_id,
  ...
  'worker_surface_split',old.id,jsonb_build_object(
    'task_key','anna_20260812_wrapping_station_stage_supplies','display_family','Wrapping Station','operation_family','Wrapping Station','display_action','Stage',
  ...
  old.created_by_user_id,old.origin_kind,old.work_lane,old.commitment_kind,'fulfill_prepare','worker_surface_grammar_v1','farm_operation',
  now(),'split_real_operation',now(),now()
from atlas.tasks old
where old.metadata->>'task_key'='anna_20260812_prep_bloom_bar_coffee_wrapping'
  and old.status in ('open','blocked')
...
select parent.farm_id,parent.organization_id,parent.zone_id,step.title,'checklist_step','open',parent.priority,parent.due_date,
  'station_checklist',parent.id,jsonb_build_object(
    'task_key',step.task_key,'stateful_child',true,'worker_execution_child',true,'checklist_group_label','Cold brew','checklist_label',step.label,'checklist_action_label','Mark complete'
  ),'check','light',parent.id,'assigned_worker',parent.assigned_membership_id,parent.assigned_user_id,
  parent.created_by_user_id,parent.origin_kind,parent.work_lane,parent.commitment_kind,'host_prepare','worker_surface_grammar_v1','farm_operation',
  now(),'station_checklist',now(),now()
from atlas.tasks parent
...
select parent.farm_id,parent.organization_id,parent.zone_id,step.title,'checklist_step','open',parent.priority,parent.due_date,
  'station_checklist',parent.id,jsonb_build_object(
    'task_key',step.task_key,'stateful_child',true,'worker_execution_child',true,'checklist_group_label','Station','checklist_label',step.label,'checklist_action_label','Mark staged'
  ),'stage','light',parent.id,'assigned_worker',parent.assigned_membership_id,parent.assigned_user_id,
  parent.created_by_user_id,parent.origin_kind,parent.work_lane,parent.commitment_kind,'fulfill_prepare','worker_surface_grammar_v1','farm_operation',
  now(),'station_checklist',now(),now()
from atlas.tasks parent
cross join lateral (values
