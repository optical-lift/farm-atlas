create or replace function atlas.worker_next_up_v2(
  p_farm_id uuid,
...
begin
  ...
  with candidate_base as (
    ...
    where t.farm_id=p_farm_id and t.assigned_membership_id=p_membership_id and t.task_scope='farm_operation' and t.status='open'
    ...
    select t.* from temporally_lawful t where coalesce((t.execution_readiness->>'ready')::boolean,false)
  ), blocked as (
    select t.* from temporally_lawful t where not coalesce((t.execution_readiness->>'ready')::boolean,false)
  ), ranked as (
    select e.*,
  ...
  select
    ...
    coalesce((select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      'taskId',b.task_id,'title',b.title,'dueDate',b.due_date,'protectedFarmMinimum',b.protected_minimum->'protectedFarmMinimum',
      'protectedCategory',b.protected_minimum->>'category','executionReadiness',b.execution_readiness,
      'operationClass',b.traits->>'operationClass','dayWindow',b.traits->>'dayWindow','environment',b.traits->>'environment'
    )) order by coalesce((b.protected_minimum->>'protectedFarmMinimum')::boolean,false) desc,b.due_date nulls last,b.title) from blocked b),'[]'::jsonb),
  ...
  if v_current_reservation is not null then
    ...
    v_state:='readiness_resolution_required'; v_blocker:='Work is due, but execution readiness is unresolved.';
...
revoke all on function atlas.worker_next_up_v2(uuid,uuid,date) from public, anon, authenticated;
grant execute on function atlas.worker_next_up_v2(uuid,uuid,date) to service_role;

create or replace function atlas.worker_self_next_up_api_v1(p_farm_id uuid,p_membership_id uuid,p_day date default null)
...
begin
  ...
  if not exists(select 1 from atlas.farm_memberships fm where fm.id=p_membership_id and fm.farm_id=p_farm_id and fm.user_id=auth.uid() and fm.active=true and fm.role='farm_hand') then raise exception 'Next Up may only be read by that active Farm Hand.' using errcode='42501'; end if;
  return atlas.worker_next_up_v2(p_farm_id,p_membership_id,p_day);
end;$$;

create or replace function atlas.owner_worker_next_up_api_v1(p_farm_id uuid,p_membership_id uuid,p_day date default null)
returns jsonb language plpgsql stable security definer set search_path=pg_catalog,atlas,auth as $$
begin
  if auth.uid() is null then raise exception 'Authenticated user required.' using errcode='42501'; end if;
