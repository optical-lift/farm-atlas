-- Prepared membership-scoped read model for the bounded weeding pilot.
-- Build 5 may place this truth into the Day spread without recalculating it in UI code.
  ...
  select jsonb_build_object(
    ...
    'subjects', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'physicalCondition', jsonb_build_object(
        ...
        'source', maintenance.estimate_source,
...
comment on function atlas.weeding_rhythm_pilot_v1(uuid) is
  'Prepared Build 4 pilot reader. Explains which Owner rule governs each pilot object, its lease boundaries, qualifying evidence, consequence policy, and current canonical task.';