...
declare
  ...
  v_unoperationalized_count integer := 0;
  v_items jsonb := '[]'::jsonb;
  v_uncovered jsonb := '[]'::jsonb;
  v_unoperationalized jsonb := '[]'::jsonb;
  v_req jsonb;
  v_result jsonb;
  v_state text;
begin
  ...
  with current_population as (
    ...
    select cp.*,
           atlas.crop_cycle_reality_expression_v8(cp.id) as reality,
           atlas.crop_cycle_requirement_expression_v1(cp.id,v_day) as requirement
    ...
    select e.*,
           ...
           coalesce((e.requirement#>>'{executionWarrant,requirementExists}')::boolean,false) as requirement_exists,
           jsonb_array_length(coalesce(e.requirement->'truthAcquisition','[]'::jsonb)) > 0 as acquisition_exists,
           coalesce((e.reality#>>'{continuity,linkedDownstreamTaskCount}')::integer,0) > 0 as linked_task_exists
    from evaluated e
  ), normalized as (
    select c.*,
           (c.disposition_class='uncovered' or c.silent_nothing) as uncovered,
           (
             c.disposition_class in ('management_or_observation_attention','acquisition_or_source_attention')
             and not c.requirement_exists
             and not c.acquisition_exists
             and not c.linked_task_exists
    ...
    select jsonb_build_object(
      ...
      'requirementExists',requirement_exists,
      'truthAcquisitionExists',acquisition_exists,
      'linkedTaskExists',linked_task_exists,
      'unoperationalizedDisposition',unoperationalized
    ) as item,
    *
    from normalized
  )
  select
    ...
    count(*) filter(where uncovered)::integer,
    ...
    count(*) filter(where unoperationalized)::integer,
    coalesce(jsonb_agg(item order by crop_label,id),'[]'::jsonb),
    coalesce(jsonb_agg(item order by crop_label,id) filter(where uncovered),'[]'::jsonb),
    coalesce(jsonb_agg(item order by crop_label,id) filter(where unoperationalized),'[]'::jsonb)
  into v_population_count,v_classified_count,v_uncovered_count,
       ...
       v_unoperationalized_count,v_items,v_uncovered,v_unoperationalized
  from item_rows;
  ...
  v_state := case
    ...
    when coalesce((v_req#>>'{summary,highPriorityIssueCount}')::integer,0) > 0
      or coalesce((v_result#>>'{summary,highPriorityIssueCount}')::integer,0) > 0
      or v_unoperationalized_count > 0 then 'covered_with_actionable_continuity_work'
    when coalesce((v_req#>>'{summary,mediumPriorityIssueCount}')::integer,0) > 0
      or coalesce((v_result#>>'{summary,mediumPriorityIssueCount}')::integer,0) > 0 then 'covered_with_continuity_attention'
  ...
  return jsonb_build_object(
    ...
    'dispositionSummary',jsonb_build_object(
      ...
      'unoperationalizedDispositionCount',v_unoperationalized_count
    ),
    'currentSubjects',v_items,
    'coverageIntegrityFailures',v_uncovered,
    'unoperationalizedDispositions',v_unoperationalized,
