-- Bed-readiness Weed Cards are enqueued before their execution Task exists.
-- The serial capture trigger previously returned as soon as it found that
-- pre-existing queue item, which meant the materialized Task never received
-- weed_serial_gate=true. Completion could therefore bypass the serial
-- progression trigger and leave a completed queue head permanently active.
--
-- Preserve the existing queue identity, but always stamp the execution carrier
-- with the serial-gate/provenance metadata it needs to advance the queue.
...
begin
  ...
  -- A planned occurrence may already own its queue position before a Task is
  -- materialized. That is not a reason to skip serial instrumentation on the
  -- execution carrier. Reuse the queue row and stamp the Task so its eventual
  -- completion crosses the predecessor-completion boundary normally.
