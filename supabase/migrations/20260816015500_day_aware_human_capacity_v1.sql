...
-- Preserve the existing boolean helper, but partial unavailability must not make the
-- entire worker day disappear.
...
-- One normalized read seam for time that is unavailable to ordinary Clock work.
-- Whole-day unavailability is intentionally excluded here because it is represented by
-- worker_day_available_v1 / fullDayUnavailable in the capacity state.
...
-- Canonical day-aware capacity state. The caller supplies the proposed/assembled paid
-- minutes because an assembled Worker Day can contain real obligations that have not yet
-- been persisted as Clock placements. Persisted placement intervals are still inspected
-- independently for time collisions.
...
-- Keep the current plan-enrichment seam so clients do not need another scheduler API.
-- The implementation now asks the day-aware capacity service and preserves its conflict
-- codes verbatim instead of manufacturing only a maximum-minute conflict.
