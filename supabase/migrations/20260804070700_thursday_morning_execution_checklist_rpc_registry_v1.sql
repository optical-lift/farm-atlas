...
with desired(signature, classification, call_site, authorization_rule) as (
  values
    ('atlas.task_execution_checklist_v1(uuid, uuid)', 'app_endpoint', 'visible execution checklist reader', 'assigned worker or visible selected operator account'),
    ('atlas.record_task_execution_check_v1(uuid, text, boolean, text, uuid)', 'app_endpoint', 'visible execution checklist item toggle', 'assigned worker or visible selected operator account')
), actual as (
  select
...
select
  ...
  1,
  jsonb_build_object(
    'source', 'thursday_morning_execution_checklist_rpc_registry_v1',
...
begin
  ...
  if v_missing <> 0 then
    raise exception 'Thursday morning execution checklist RPC registry reconciliation is incomplete: % missing.', v_missing;
  end if;
end;
$verify$;
