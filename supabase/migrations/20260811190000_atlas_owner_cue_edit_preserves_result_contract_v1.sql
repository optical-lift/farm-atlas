-- Owner cue editing preservation v1
-- Owner can move/edit choreography without accidentally erasing a generated
-- farm-state result contract when the UI is not explicitly changing that contract.
...
begin
  ...
  begin v_id:=nullif(p_cue->>'cueId','')::uuid;
  exception when invalid_text_representation then raise exception 'cueId must be a UUID.' using errcode='22023'; end;
  begin v_service_date:=nullif(p_cue->>'serviceDate','')::date;
  exception when others then raise exception 'serviceDate must be a date.' using errcode='22023'; end;
  v_kind:=nullif(p_cue->>'cueKind','');
  v_anchor:=nullif(p_cue->>'anchorKind','');
  begin v_anchor_task_id:=nullif(p_cue->>'anchorTaskId','')::uuid;
  exception when invalid_text_representation then raise exception 'anchorTaskId must be a UUID.' using errcode='22023'; end;
  ...
  v_has_result_contract:=p_cue ? 'resultContract';
  ...
  begin v_scheduled_at:=nullif(p_cue->>'scheduledAt','')::timestamptz;
  exception when others then raise exception 'scheduledAt is invalid.' using errcode='22023'; end;
  begin v_available_from:=nullif(p_cue->>'availableFrom','')::timestamptz;
  exception when others then raise exception 'availableFrom is invalid.' using errcode='22023'; end;
  begin v_expires_at:=nullif(p_cue->>'expiresAt','')::timestamptz;
  exception when others then raise exception 'expiresAt is invalid.' using errcode='22023'; end;
  ...
  if v_anchor_task_id is not null and not exists (
    ...
    where task.id=v_anchor_task_id
  ...
  if v_id is null then
    ...
    set service_date=v_service_date,
        ...
        body=v_body,
        payload=v_payload,
        result_contract=case when v_has_result_contract then v_result_contract else cue.result_contract end,
