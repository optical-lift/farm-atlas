...
begin
  ...
  -- Linda's Flowers: preserve only the worker-relevant reconnect cue; historical
  -- account-volume reasoning remains private.
  update atlas.tasks t
  set metadata=coalesce(t.metadata,'{}'::jsonb)||jsonb_build_object(
  ...
  -- Creeping thyme is one truthful batch: 200 + 200 + 80 = 480, not 600.
  select t.* into v_task from atlas.tasks t
  ...
  -- Pool-path hardscape relocation already has exact source + destination steps.
  select t.* into v_task from atlas.tasks t
  ...
  -- Sutton's Apricot foxglove single-tray move.
  select t.* into v_task from atlas.tasks t
