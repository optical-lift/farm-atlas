-- Fixed calendar work keeps its authored date even when the release engine runs late.
...
begin
  ...
  set work_lane = 'rhythm',
      ...
      task_payload = coalesce(task_payload, '{}'::jsonb)
        || jsonb_build_object('commitment_kind', 'hard_date'),
      metadata = coalesce(metadata, '{}'::jsonb)
        || jsonb_build_object('dateBehavior', 'hard_date'),
      updated_at = now()
...
begin
  if new.planned_occurrence_id is null then
    return new;
  end if;
  ...
  new.task_series_key := coalesce(
    new.task_series_key,
    nullif(v_occurrence.task_payload ->> 'task_series_key', '')
  );
  new.engine_instance_key := coalesce(
    new.engine_instance_key,
    nullif(v_occurrence.task_payload ->> 'engine_instance_key', ''),
...
drop trigger if exists a_hydrate_fixed_calendar_task_identity_v1 on atlas.tasks;
create trigger a_hydrate_fixed_calendar_task_identity_v1
before insert on atlas.tasks
for each row execute function atlas.hydrate_fixed_calendar_task_identity_v1();
...
begin
  ...
  set task_series_key=coalesce(task.task_series_key,nullif(occurrence.task_payload->>'task_series_key','')),
      engine_instance_key=coalesce(task.engine_instance_key,nullif(occurrence.task_payload->>'engine_instance_key',''),occurrence.occurrence_key),
      commitment_kind='hard_date',
