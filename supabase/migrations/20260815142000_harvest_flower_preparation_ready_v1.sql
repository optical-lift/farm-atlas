-- Harvest Pass 4: harvested physical output -> completed preparation -> Ready inventory.
--
-- This is deliberately separate from the unused stem-count postharvest custody engine.
-- Harvest observations remain physical field output. A preparation result consumes those
-- observations once, records the real handling/transformation that occurred, and may create
-- saleable Ready lots. Nothing becomes Ready merely because it was harvested.
...
begin
  ...
  -- Validate output semantics before consuming any harvest observations.
