do $$
begin
  if to_regprocedure('atlas.record_task_transition_v1_internal_legacy(uuid,text,text,date,text,text,text,text,jsonb,uuid)') is null
     and to_regprocedure('atlas.record_task_transition_v1_internal(uuid,text,text,date,text,text,text,text,jsonb,uuid)') is not null then
    execute 'alter function atlas.record_task_transition_v1_internal(uuid,text,text,date,text,text,text,text,jsonb,uuid) rename to record_task_transition_v1_internal_legacy';
  end if;
end $$;

create or replace function atlas.record_task_transition_v1_internal(
  p_task_id uuid,p_transition text,p_idempotency_key text,p_target_date date default null,
  p_note text default null,p_reason text default null,p_lane_key text default null,p_work_key text default null,
  p_payload jsonb default '{}'::jsonb,p_existing_field_log_id uuid default null
) returns jsonb
