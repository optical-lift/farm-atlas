...
where qi.farm_id='6a503d9f-4008-4ddb-b3f0-cc6ab825dc9f'::uuid and qi.queue_key='anna_weeding_rotation' and qi.position=13 and qi.planned_occurrence_id in (select id from atlas.planned_work_occurrences where title='Weed MG11' and state='planned');
...
where p.id in (select qi.planned_occurrence_id from atlas.task_release_queue_items qi where qi.farm_id='6a503d9f-4008-4ddb-b3f0-cc6ab825dc9f'::uuid and qi.queue_key='anna_weeding_rotation' and qi.position=14 and qi.state='skipped') and p.title='Weed MG11';
