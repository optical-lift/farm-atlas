-- Ticketed Thursday paid-prep variant v1
-- Paid evenings retain the common Venue prep contract while substituting
...
begin
  ...
  if v_template='community_thursday_venue_prep_v1' and v_paid then
    ...
    set metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
      'venue_prep_variant','ticketed_evening',
      'mirrors_free_morning_design',false,
      'truthBoundary','Free mornings and ticketed evenings share the Venue cycle, while event-specific requirements and drinkware remain instance-aware.'
    ),updated_at=now()
    where id=v_task.id;
    ...
    update atlas.tasks
    set metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('venue_prep_variant','free_community_morning')
    where id=v_task.id;
  end if;
...
'Thursdays at Elm Venue checklist seeder. Paid evening prep substitutes paper and clear plastic disposable drinkware for the free-morning mug-hutch presentation while retaining event-instance requirements.';
