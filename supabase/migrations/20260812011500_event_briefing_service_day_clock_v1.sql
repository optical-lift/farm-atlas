-- Dynamic event briefing language uses the event day's clock, not whatever
-- clock time happens to be current while an Owner previews a future Day.
...
begin
  ...
  if p_day=v_project.target_date
    ...
    if v_setup then
      return v_departure_label||' '||v_departure_action||' is still open. Elm setup follows. '||v_remaining::text||' moves remain before tonight.';
    end if;
    return v_departure_label||' '||v_departure_action||' is still open. '||v_remaining::text||' moves remain before tonight.';
  end if;

  if v_departure_label is not null and v_setup then
    return v_departure_label||' '||v_departure_action||' this morning. Elm setup afterward. '||v_remaining::text||' moves make tonight ready.';
  end if;
  if v_departure_label is not null then
    return v_departure_label||' '||v_departure_action||' is the first move. '||v_remaining::text||' moves remain for tonight.';
  end if;
  if v_setup then
