...
-- current_principal_id_v1 is a composition helper for SECURITY DEFINER Principal endpoints.
-- Signed-in clients do not need a direct identity-RPC surface.
...
(
  ...
  jsonb_build_object(
    'purpose','Return the authenticated Principal capacity-policy inventory and resolved capacity today',
    'boundary','auth.uid resolves through internal current_principal_id_v1; no cross-Principal identifier input'
  ),now()
),
(
  ...
  jsonb_build_object(
    'purpose','Resolve auth.uid to the active Principal id for trusted Principal composition functions',
    'boundary','internal composition helper; direct authenticated EXECUTE intentionally revoked'
  ),now()
)
