...
begin
  ...
  if not exists (select 1 from atlas.farm_memberships fm where fm.farm_id=p_farm_id and fm.active=true and fm.role='owner' and fm.user_id=auth.uid()) then
  ...
  if exists (select 1 from jsonb_array_elements(p_changes) c group by c->>'taskId' having count(*) > 1) then
  ...
  loop
    ...
    select * into v_task from atlas.tasks t where t.id=v_task_id and t.farm_id=p_farm_id and t.assigned_membership_id=p_membership_id;
    if v_task.id is null then raise exception 'The selected task is not assigned to this worker.' using errcode='55000'; end if;
    if lower(coalesce(v_task.status::text,'')) in ('done','completed','archived','skipped') then raise exception 'This task is no longer open for Clock planning.' using errcode='55000'; end if;

    select * into v_existing from atlas.worker_day_task_placements p where p.task_id=v_task_id for update;
    if v_existing.id is not null and (v_existing.state <> 'placed' or v_existing.service_date <> p_day) then raise exception 'This task is not placed on the selected worker day.' using errcode='55000'; end if;
    ...
    if v_set_start then
      ...
      if v_start_text is null then v_start_at := null;
      else
    ...
    if v_existing.id is null then
      ...
      insert into atlas.worker_day_task_placements(organization_id,farm_id,membership_id,task_id,service_date,day_window,sort_order,placement_source,placement_reason,state,owner_actor_user_id,planned_start_at,planned_duration_minutes)
      values (v_task.organization_id,p_farm_id,p_membership_id,v_task_id,p_day,v_day_window,v_sort_order,'owner','Committed from Owner Clock plan.','placed',auth.uid(),v_start_at,v_duration)
      returning * into v_after;
    else
      update atlas.worker_day_task_placements p set planned_start_at=v_start_at,planned_duration_minutes=v_duration,
    ...
    insert into atlas.worker_day_task_placement_events(organization_id,farm_id,membership_id,task_id,placement_id,event_kind,from_service_date,to_service_date,from_day_window,to_day_window,from_sort_order,to_sort_order,actor_user_id,metadata)
    values (v_task.organization_id,p_farm_id,p_membership_id,v_task_id,v_after.id,'owner_clock_plan_commit',v_existing.service_date,v_after.service_date,v_existing.day_window,v_after.day_window,v_existing.sort_order,v_after.sort_order,auth.uid(),
...
insert into atlas.authenticated_rpc_registry(
  ...
  'atlas.owner_commit_worker_clock_plan_api_v1(uuid, uuid, date, jsonb)','owner_admin_endpoint','verified','active',true,true,true,1,0,
  jsonb_build_object('route','/api/atlas/owner-clock-plan-commit','purpose','Atomically commit Owner Clock start/duration choreography','boundary','Owner-only; updates worker-day placement truth only','taskTruth','does not update atlas.tasks, task due dates, or Task Focus result state','rollback','any stale placement, invalid change, or failed write rolls back the whole Clock plan'),now()
)
