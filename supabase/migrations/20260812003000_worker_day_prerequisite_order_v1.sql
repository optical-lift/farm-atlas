-- Worker Day should lead with work the farm hand can actually perform. A
-- downstream task that is blocked only because a canonical prerequisite is still
-- incomplete remains true farm/project state, but it is held out of the worker's
-- actionable Day until that prerequisite is satisfied. Owner/project views keep
