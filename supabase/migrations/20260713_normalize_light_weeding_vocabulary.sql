-- Atlas: describe the protected daily bed assignment as light weeding.
update atlas.maintenance_scheduler_settings s
set metadata = (
      coalesce(s.metadata, '{}'::jsonb)
      - 'daily_quick_pass_enabled'
      - 'daily_quick_pass_minutes'
      - 'daily_quick_pass_conditions'
      - 'daily_quick_pass_object_type'
      - 'daily_quick_pass_tools'
      - 'daily_quick_pass_mode'
      - 'daily_quick_pass_seedling_safety_days'
    ) || jsonb_build_object(
      'daily_light_pass_enabled', true,
      'daily_light_pass_minutes', 20,
      'daily_light_pass_conditions', jsonb_build_array('maintained', 'moderate'),
      'daily_light_pass_object_type', 'bed',
      'daily_light_pass_mode', 'bonus_timebox',
      'daily_light_pass_seedling_safety_days', 21
    ),
    updated_at = now()
where s.farm_id = (
...
declare
  ...
  v_light_pass_enabled boolean := false;
  v_light_pass_minutes integer := 20;
  v_light_pass_effort integer;
  v_date date;
  v_day integer;
...
begin
  ...
  v_light_pass_enabled :=
    p_maintenance_type = 'weed'
    and lower(coalesce(v_settings_metadata->>'daily_light_pass_enabled', 'false')) = 'true';

  v_light_pass_minutes := case
    when coalesce(v_settings_metadata->>'daily_light_pass_minutes', '') ~ '^[0-9]+$'
      then greatest(5, least(60, (v_settings_metadata->>'daily_light_pass_minutes')::integer))
    else 20
  end;
  ...
  for v_day in 0..p_days - 1 loop
    ...
    if v_light_pass_enabled then
      select
        ...
        array_remove(array[
          'daily light weeding pass',
          'light weeding assignment',
      ...
      if found then
        v_light_pass_effort := least(v_light_pass_minutes, v_rec.effort_minutes);
        v_evening_capacity := v_evening_capacity + v_light_pass_effort;
        v_evening_left := v_evening_left + v_light_pass_effort;
        ...
        condition := 'light';
        estimated_minutes := v_light_pass_effort;
        ...
        estimate_source := 'daily_light_timebox';
...
comment on function atlas.preview_maintenance_schedule(text, date, integer, text) is
  'Packs canonical maintenance into daily labor windows and reserves one extra 20-minute light-weeding bed during weeding rotations. Newly planted, germinating, sparse, emerging, and recent partial stands are excluded from the light pass.';
...
begin
  ...
  loop
    v_is_light := 'daily light weeding pass' = any(coalesce(r.priority_reasons, '{}'::text[]));
    ...
    insert into atlas.tasks(
      ...
      jsonb_build_object(
        ...
        'display_detail', case when v_is_light
          then ''
        ...
        'condition', case when v_is_light then 'light' else r.condition end,
        'effort_band', case
          when v_is_light then 'light'
          when r.condition in ('reset', 'heavy') then 'heavy'
          when r.condition = 'moderate' then 'moderate'
          else 'light'
        end,
        ...
        'timeboxed_minutes', case when v_is_light then r.estimated_minutes else null end,
        'seedling_safety_rule', case when v_is_light then 'established crop rows only' else null end,
...
-- Normalize the already-issued light task without prescribing a tool or technique.
update atlas.tasks
set title = 'Weed Berry Walk Bed 2',
    unlock_text = null,
    note = null,
    metadata = (
      coalesce(metadata, '{}'::jsonb)
      - 'tool_hint'
      - 'quick_maintenance_pass'
    ) || jsonb_build_object(
      'task_key', 'daily_light_weeding_bw2_20260713',
      'condition', 'light',
      'effort_band', 'light',
      'display_action', 'Weed',
      'display_title', 'Weed Berry Walk Bed 2',
      'display_detail', '',
      'priority_reasons', jsonb_build_array(
        'daily light weeding pass',
        'light weeding assignment',
        'established crop rows'
      ),
    ...
    updated_at = now()
where id = '724e4aaa-524c-46a7-8126-f02f6b6726b2';
