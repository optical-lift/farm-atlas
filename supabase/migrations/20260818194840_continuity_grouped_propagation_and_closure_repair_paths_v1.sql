...
select
  ...
  jsonb_build_object(
    ...
    'display_action','Inspect + reclassify',
    'display_subject','Grow Room propagation bodies',
...
with farm as (
  ...
  select elem
  from audit a cross join lateral jsonb_array_elements(a.payload->'issueFamilies') elem
...
select
  rt.id,c.crop_cycle_id,'observes','confirmed','continuity_repair_v1',
  jsonb_build_object(
    'repairPacketKey','audit:propagation_transition_uncovered',
...
select
  f.id,f.organization_id,
  'Close First Lady Snapdragon cleanup state',
  'crop_cleanup','open','high',null,'continuity_repair',
  jsonb_build_object(
    'task_key','continuity_20260818_first_lady_snapdragon_cleanup',
    'repair_packet_key','audit:closure_uncovered',
    'repair_owner_function','farm_operations',
    ...
    'created_source','continuity_interlock_audit_20260818',
    'display_action','Clean up + close',
    'display_subject','First Lady Mixed Colors Snapdragon',
    'display_location','Grow Room seed shelves',
    'work_route','clear_reconcile'
  ),
...
insert into atlas.task_crop_cycles(task_id,crop_cycle_id,role,confidence,source,metadata)
select
  rt.id,c.crop_cycle_id,'clears','confirmed','continuity_repair_v1',
  jsonb_build_object(
    'repairPacketKey','audit:closure_uncovered',
