-- Capacity arithmetic remains in owner-only planning tables and readers.
-- Task cards are the worker-facing operational contract and must not carry it.
...
select
  ...
  coalesce((
    ...
    where fl.farm_id=t.farm_id and fl.metadata->>'task_id'=t.id::text
  ),'[]'::jsonb) as task_logs,
  coalesce((
    select jsonb_agg(jsonb_build_object(
      'event_id',toe.id,
...
comment on view atlas.v_task_cards is
  'Worker-safe task card projection. Capacity, duration, physical-load, unit-budget, and recovery-planning metadata remain owner-private.';
