...
begin
  ...
  -- The day counter follows the work actually visible for this date. It is not a
  -- frozen morning snapshot: work that appears later and work completed later
  -- both remain part of the same truthful day.
  ...
  if v_day = v_today then
    insert into atlas.day_plan_snapshots (
      ...
      v_flexible_ids,
      '{}'::uuid[],
      cardinality(v_carryover_ids),
      0,
      jsonb_build_object(
        'contract_version', 'living_day_plan_v2',
        'denominator_rule', 'all_visible_due_work',
    ...
    select snapshot.id, snapshot.prepared_at
    into v_snapshot_id, v_prepared_at
  ...
  select coalesce(array_agg(task.id order by array_position(v_candidate_ids, task.id)), '{}'::uuid[])
  into v_resolved_ids
  from atlas.tasks task
  where task.id = any(v_candidate_ids)
    and (
      task.status = 'done'
      or exists (
        select 1
  ...
  return jsonb_build_object(
    'contractVersion', 'living_day_plan_v2',
    'farmId', p_farm_id,
