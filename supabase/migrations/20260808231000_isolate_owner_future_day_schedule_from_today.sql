...
BEGIN
  ...
  IF p_done_date IS NOT NULL AND p_done_date > v_today THEN
    RETURN QUERY
    SELECT card.*
    FROM atlas.presented_work_rows_v1(v_farm_id, v_membership_id, p_done_date) presented
    JOIN atlas.v_task_cards card ON card.task_id = presented.task_id
    WHERE presented.presentation_state IN ('attention', 'presented')
      AND card.status IN ('open', 'blocked', 'done')
    ORDER BY presented.lane_order, presented.selection_rank, card.task_id;
    RETURN;
  END IF;
