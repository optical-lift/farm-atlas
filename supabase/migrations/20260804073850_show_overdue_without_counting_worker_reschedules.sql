-- Overdue work remains visible to its assigned worker. A worker-authored
-- reschedule preserves the backlog card without charging its minutes to the
-- current day's selected workload.
...
comment on function atlas.task_rescheduled_by_membership_v1(uuid, uuid, text) is
  'Internal resolver: true when the target membership authored a reschedule transition for the task.';
...
-- Restore the dates selected through Anna's task page. This is a correction of
-- schedule authority, not a new task transition.
with elm as (
  select id
  from atlas.farms
  ...
  select
    task.id,
    case
      when task.metadata ->> 'task_key' = 'anna_20260728_put_away_cat_litter_food'
        then date '2026-07-29'
      when task.title = 'Restore mowing rhythm — Field Rows · Back Half'
        then date '2026-07-29'
      when task.metadata ->> 'task_key' = 'anna_20260726_set_up_fan_seedlings'
        then date '2026-08-03'
    end as target_date
  ...
  set due_date = targets.target_date,
      metadata = (
