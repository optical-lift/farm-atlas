...
begin
  ...
  if v_boundary.id is null
     ...
     or v_boundary.requirement_set_key<>'anna_weeding_predecessor_completion_v1'
    ...
    raise exception 'Boundary event does not authorize an Anna weeding serial release effect.' using errcode='23514';
  end if;
  ...
  if v_item.id is null
     or v_item.queue_key<>'anna_weeding_rotation'
     or v_item.task_id is distinct from v_boundary.source_id then
    raise exception 'Boundary subject does not match the Anna weeding queue item.' using errcode='23514';
  end if;
  ...
  if v_item.state='completed' then
    if nullif(v_item.metadata->>'completion_boundary_event_id','')::uuid is distinct from v_boundary.id then
      raise exception 'Anna weeding queue item was completed by a different boundary.' using errcode='23514';
    end if;
    return jsonb_build_object(
      'contractVersion','anna_weeding_serial_release_effect_v1',
      'applied',false,
  ...
  if v_item.state<>'active' then
    raise exception 'Anna weeding effect requires the boundary subject to be active.' using errcode='23514';
  end if;
  ...
  if v_task.id is null or v_task.status<>'done' then
    raise exception 'Anna weeding predecessor completion is not authoritative.' using errcode='23514';
  end if;
  ...
  set state='completed',
      ...
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
        ...
        'effect_contract','anna_weeding_serial_release_effect_v1'
      )
  where id=v_item.id;
...
begin
  ...
  if lower(coalesce(new.metadata->>'weed_serial_gate','false')) not in ('true','yes','1') then return new; end if;
  ...
  v_boundary_id:=atlas.record_requirement_boundary_v1(
    ...
    v_item.id,
    'anna_weeding_predecessor_completion_v1',
    'anna-weeding:'||v_item.id::text||':predecessor-completed',
    v_before,
    v_after,
    v_evaluated_at,
    'task',
    ...
    jsonb_build_object(
      ...
      'effectConsumer','apply_anna_weeding_serial_release_effect_v1'
    )
  );

  if v_boundary_id is null then
    raise exception 'Anna weeding predecessor completion did not cross an open-to-satisfied boundary.' using errcode='23514';
  end if;

  v_completed_date:=(v_evaluated_at at time zone 'America/Chicago')::date;
  perform atlas.apply_anna_weeding_serial_release_effect_v1(v_boundary_id,v_completed_date);
  return new;
end;
$function$;
...
declare
  ...
  v_is_anna_weeding boolean:=false;
...
begin
  ...
  v_is_anna_weeding:=p_queue_key='anna_weeding_rotation';

  if v_is_pot_up_serial or v_is_owner_completion_gated or v_is_anna_weeding then
    v_expected_requirement_set_key:=case
      when v_is_pot_up_serial then 'pot_up_serial_predecessor_completion_v1'
      when v_is_owner_completion_gated then 'owner_completion_gated_predecessor_completion_v1'
      else 'anna_weeding_predecessor_completion_v1'
    end;
    ...
    set task_id=v_task_id,
        ...
        metadata=(coalesce(metadata,'{}'::jsonb)-'awaiting_owner_schedule_approval'-'awaiting_owner_schedule_approval_at'-'release_waiting_on_capacity')||jsonb_build_object(
          ...
          'release_architecture',case
            ...
            when v_is_anna_weeding then 'boundary_authorized_completion_gated_release_v1'
...
revoke execute on function atlas.release_next_task_in_queue_v1(uuid,text,date) from public, anon, authenticated;
