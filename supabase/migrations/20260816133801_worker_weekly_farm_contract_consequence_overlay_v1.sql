...
declare
  ...
  v_required_dependency_link_missing integer:=0;
  v_missed_hard_dates jsonb:='[]'::jsonb;
  v_missed_hard_date_count integer:=0;
  v_expired_lawful_window_work jsonb:='[]'::jsonb;
  v_expired_lawful_window_count integer:=0;
  v_state text;
...
begin
  ...
  with candidate_base as (
    ...
    select s.*,
      case
        when (s.latest_lawful_date is not null and s.latest_lawful_date<v_week_start)
          or (s.hard_finish_date is not null and s.hard_finish_date<v_week_start)
          then 'expired_lawful_window'
        when s.hard_date_contract and s.due_date is not null and s.due_date<v_week_start and not s.committed_in_week
          then 'missed_hard_date'
        else null
    ...
    select c.*,atlas.task_effective_delay_consequence_v1(c.task_id,v_week_start) as effective_consequence
    from classified c
  ), decorated as (
    select c.*,
      ('[]'::jsonb
    ...
    select
      coalesce(jsonb_agg(jsonb_build_object(
        ...
        'effectiveConsequenceTier',case when coalesce(d.effective_consequence->>'effectiveTier','') ~ '^[1-6]$' then (d.effective_consequence->>'effectiveTier')::integer else null end,
        'effectiveConsequenceClass',d.effective_consequence->>'effectiveClass',
        'effectiveConsequenceSource',d.effective_consequence->>'effectiveSource',
        'consequenceNeedsResolution',coalesce((d.effective_consequence->>'needsConsequenceResolution')::boolean,true),
        'dependencyLinkMissing',coalesce((d.effective_consequence->>'dependencyLinkMissing')::boolean,false),
        'inheritedConsequenceFromTaskId',nullif(d.effective_consequence->>'inheritedFromTaskId','')::uuid,
        'committedInWeek',d.committed_in_week
      ) order by
        d.required_this_week desc,
        case when coalesce(d.effective_consequence->>'effectiveTier','') ~ '^[1-6]$' then (d.effective_consequence->>'effectiveTier')::integer else 99 end,
        d.hard_finish_date nulls last,d.latest_lawful_date nulls last,d.due_date nulls last,d.title,d.task_id
      ),'[]'::jsonb) as work,
      coalesce(sum(d.expected_active_minutes) filter(where d.required_this_week and d.expected_active_minutes>0),0)::integer as required_minutes,
      count(*) filter(where d.required_this_week and d.expected_active_minutes<=0)::integer as required_unestimated,
      count(*) filter(where d.required_this_week)::integer as required_count,
      count(*) filter(where d.required_this_week and (d.status='blocked' or not d.prerequisites_ready or not d.resources_ready))::integer as required_readiness_risks,
      coalesce(sum(d.expected_active_minutes) filter(where not d.required_this_week and d.expected_active_minutes>0),0)::integer as optional_minutes,
      count(*) filter(where not d.required_this_week)::integer as optional_count,
      count(*) filter(where d.required_this_week and coalesce((d.effective_consequence->>'needsConsequenceResolution')::boolean,true))::integer as required_consequence_unresolved,
      count(*) filter(where d.required_this_week and coalesce((d.effective_consequence->>'dependencyLinkMissing')::boolean,false))::integer as required_dependency_link_missing
    from decorated d
    where d.exclusion_reason is null
  ), exception_agg as (
    select
      coalesce(jsonb_agg(jsonb_build_object(
        'taskId',d.task_id,'title',d.title,'dueDate',d.due_date,'commitmentKind',d.commitment_kind,
        'reason','past_hard_date_requires_explicit_reauthorization','expectedActiveMinutes',d.expected_active_minutes,
        'sourceKind',d.source_kind,'plannedOccurrenceId',d.planned_occurrence_id
      ) order by d.due_date,d.title,d.task_id) filter(where d.exclusion_reason='missed_hard_date'),'[]'::jsonb) as missed_hard_dates,
      count(*) filter(where d.exclusion_reason='missed_hard_date')::integer as missed_hard_date_count,
      coalesce(jsonb_agg(jsonb_build_object(
        'taskId',d.task_id,'title',d.title,'dueDate',d.due_date,'latestLawfulDate',d.latest_lawful_date,
        'hardFinishDate',d.hard_finish_date,'reason','lawful_window_expired_clock_cannot_carry',
        'expectedActiveMinutes',d.expected_active_minutes,'sourceKind',d.source_kind,'plannedOccurrenceId',d.planned_occurrence_id
      ) order by coalesce(d.latest_lawful_date,d.hard_finish_date,d.due_date),d.title,d.task_id)
        filter(where d.exclusion_reason='expired_lawful_window'),'[]'::jsonb) as expired_lawful_window_work,
      count(*) filter(where d.exclusion_reason='expired_lawful_window')::integer as expired_lawful_window_count
    from decorated d
  )
  select
    w.work,w.required_minutes,w.required_unestimated,w.required_count,w.required_readiness_risks,w.optional_minutes,w.optional_count,
    w.required_consequence_unresolved,w.required_dependency_link_missing,
    e.missed_hard_dates,e.missed_hard_date_count,e.expired_lawful_window_work,e.expired_lawful_window_count
  into
    v_work,v_required_minutes,v_required_unestimated,v_required_count,v_required_readiness_risks,v_optional_minutes,v_optional_count,
    v_required_consequence_unresolved,v_required_dependency_link_missing,
    v_missed_hard_dates,v_missed_hard_date_count,v_expired_lawful_window_work,v_expired_lawful_window_count
  from work_agg w cross join exception_agg e;
  ...
  return jsonb_build_object(
    ...
    'requiredDependencyLinkMissingCount',v_required_dependency_link_missing,
    'optionalCandidateCount',v_optional_count,'optionalCandidateEstimatedMinutes',v_optional_minutes,
    'missedPastHardDateCount',v_missed_hard_date_count,'missedPastHardDates',v_missed_hard_dates,
    'expiredLawfulWindowCount',v_expired_lawful_window_count,'expiredLawfulWindowWork',v_expired_lawful_window_work,
    'hasTemporalExceptions',(v_missed_hard_date_count+v_expired_lawful_window_count)>0,
