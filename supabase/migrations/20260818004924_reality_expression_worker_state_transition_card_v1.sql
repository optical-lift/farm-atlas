...
begin
  ...
  loop
    ...
    v_fit_function := nullif(v_packet #>> '{fittingOperation,function}','');
    ...
    if v_fit_state <> 'ready' or coalesce(v_attention_count,0)>0 then
      v_resolution_count := v_resolution_count + 1;
    end if;
    if v_fit_state <> 'ready' or v_fit_function is distinct from v_task.action_key then
  ...
  return jsonb_build_object(
    ...
    'fittingFunction',jsonb_build_object(
      ...
      'principle','A task label or operation class is not promoted to a fitting operation unless the linked Reality Expression independently supports the same action identity.'
    ),
...
create or replace function atlas.worker_day_state_transition_cards_v1(
  p_farm_id uuid,
  p_membership_id uuid,
  p_service_date date
)
...
begin
  ...
  select coalesce(jsonb_agg(
    atlas.worker_state_transition_card_v1(p_farm_id,p_membership_id,placement.task_id,p_service_date)
  ...
  return jsonb_build_object(
    'contractVersion','worker_day_state_transition_cards_v1',
    'farmId',p_farm_id,
    'membershipId',p_membership_id,
    'serviceDate',p_service_date,
    'cards',v_cards,
    'integrationState','diagnostic_parallel_to_legacy_worker_day',
    'principle','Phase 5 diagnoses routed work through Reality Expression before this contract is allowed to replace the legacy Worker Day card path.'
  );
end;
$function$;

revoke all on function atlas.worker_day_state_transition_cards_v1(uuid,uuid,date) from public;
revoke all on function atlas.worker_day_state_transition_cards_v1(uuid,uuid,date) from anon;
grant execute on function atlas.worker_day_state_transition_cards_v1(uuid,uuid,date) to authenticated;
grant execute on function atlas.worker_day_state_transition_cards_v1(uuid,uuid,date) to service_role;
...
values
  (
    'atlas.worker_state_transition_card_v1(uuid, uuid, uuid, date)',
    'app_endpoint','verified','active',true,true,true,0,0,
    jsonb_build_object(
      'purpose','Read one routed task as a Reality Expression state-transition card without treating the task as Source.',
      'boundary','Active target worker may read self; farm management may preview. SECURITY DEFINER is limited by explicit membership/farm authorization before service-internal Reality Expression calls.',
      'architecture','Placement proves routing/time claim; linked Crop Cycle or Production Reality Expression controls operation warrant; unresolved reality suppresses authorizedInstruction; result mutation remains Phase 6.'
    ),now()
  ),
  (
    'atlas.worker_day_state_transition_cards_v1(uuid, uuid, date)',
    'app_endpoint','verified','active',true,true,true,0,0,
    jsonb_build_object(
      'purpose','Read all canonical placed tasks for one Worker Day through the Phase 5 state-transition-card contract.',
      'boundary','Active target worker may read self; farm management may preview. Read-only diagnostic remains parallel to the legacy Worker Day until acceptance proves the new truth boundary.',
      'architecture','No task, placement, crop, Production, Clock, or result state is mutated.'
    ),now()
  )
