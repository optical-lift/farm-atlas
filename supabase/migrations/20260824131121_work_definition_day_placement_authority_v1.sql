...
set metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object(
  'dayPlacement', jsonb_build_object('window','morning','anchor','top','order',0),
  'dayPlacementAuthority','work_definition'
), updated_at=now()
where stable_key='farm_round:stewardship:v1' and active=true;
...
declare
  ...
  v_day_placement jsonb:='{}'::jsonb;
  v_placement_metadata jsonb:='{}'::jsonb;
begin
  ...
  where farm_id=p_farm_id and stable_key='farm_round:stewardship:v1' and active=true
  ...
  if v_definition.id is not null and jsonb_typeof(v_definition.metadata->'dayPlacement')='object' then
    v_day_placement:=v_definition.metadata->'dayPlacement';
    v_placement_metadata:=jsonb_strip_nulls(jsonb_build_object(
      'work_order_anchor',nullif(v_day_placement->>'anchor',''),
      'day_work_order',case when v_day_placement ? 'order' then (v_day_placement->>'order')::integer else null end,
      'day_window',nullif(v_day_placement->>'window',''),
      'day_placement_source','work_definition',
      'day_placement_definition_id',v_definition.id
    ));
  end if;
  ...
  v_payload:=jsonb_build_object(
    ...
    'created_by_user_id',v_owner.user_id,
    'task_series_key','anna_farm_round_daily','engine_instance_key','farm_round:'||p_membership_id::text||':'||p_service_date::text,
    'metadata',jsonb_build_object(
      ...
      'structured_result_required',true,'quick_complete_allowed',false,'hide_details',true,
  ...
  set work_lane='required',commitment_kind='hard_date',effort_units=0,
      miss_consequence=jsonb_build_object(
        'tier',5,
        'class','recurring_maintenance',
        'reason','The grouped stewardship members remain due until completed.'
      ),
  ...
  set parent_occurrence_id=v_parent_occurrence_id,
      task_payload=jsonb_set(
        coalesce(o.task_payload,'{}'::jsonb),'{metadata}',
      ...
      metadata=coalesce(o.metadata,'{}'::jsonb)||jsonb_build_object('farmRoundContract','farm_round_grouping_v1','farmRoundParentOccurrenceId',v_parent_occurrence_id),
      updated_at=now()
...
with cfg as (
  select id, metadata->'dayPlacement' as placement
  from atlas.work_definitions
  where stable_key='farm_round:stewardship:v1' and active=true
  ...
  set task_payload=jsonb_set(
        ...
        '{metadata}',
        coalesce(o.task_payload->'metadata','{}'::jsonb)
          - 'day_order'
          || jsonb_strip_nulls(jsonb_build_object(
              'work_order_anchor',nullif(cfg.placement->>'anchor',''),
              'day_work_order',case when cfg.placement ? 'order' then (cfg.placement->>'order')::integer else null end,
              'day_window',nullif(cfg.placement->>'window',''),
              'day_placement_source','work_definition',
              'day_placement_definition_id',cfg.id
            )),
        true
      ),
      updated_at=now()
  from cfg
  where o.work_definition_id=cfg.id and o.state<>'cancelled'
  returning o.id,o.released_task_id,cfg.id as definition_id,cfg.placement
)
update atlas.tasks t
set metadata=(coalesce(t.metadata,'{}'::jsonb) - 'day_order')
  || jsonb_strip_nulls(jsonb_build_object(
      'work_order_anchor',nullif(u.placement->>'anchor',''),
      'day_work_order',case when u.placement ? 'order' then (u.placement->>'order')::integer else null end,
      'day_window',nullif(u.placement->>'window',''),
      'day_placement_source','work_definition',
      'day_placement_definition_id',u.definition_id
    )),
    updated_at=now()
from updated_occurrences u
where u.released_task_id=t.id;
