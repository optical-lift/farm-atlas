...
declare v_farm_id uuid;v_water_due boolean;v_weed_due boolean;v_today date := (now() at time zone 'America/Chicago')::date;
begin
...
select coalesce(bool_or(water_status='needs_water'),false),coalesce(bool_or(weed_pressure in ('moderate','heavy')),false)
into v_water_due,v_weed_due from atlas.production_field_care_state where production_lot_id=p_production_lot_id and establishment_status<>'failed';
insert into atlas.production_care_policies(farm_id,production_lot_id,care_kind,policy_status,required_before_harvest,current_status,last_satisfied_at,metadata)
values(v_farm_id,p_production_lot_id,'watering','monitor',true,case when v_water_due then 'needs_attention' else 'satisfied' end,case when v_water_due then null else v_today end,jsonb_build_object('source','field_care_state','current_need',v_water_due))
on conflict(production_lot_id,care_kind) do update set policy_status='monitor',required_before_harvest=true,current_status=case when v_water_due then 'needs_attention' else 'satisfied' end,last_satisfied_at=case when v_water_due then atlas.production_care_policies.last_satisfied_at else v_today end,metadata=atlas.production_care_policies.metadata||jsonb_build_object('source','field_care_state','current_need',v_water_due),updated_at=now();
insert into atlas.production_care_policies(farm_id,production_lot_id,care_kind,policy_status,required_before_harvest,current_status,last_satisfied_at,metadata)
values(v_farm_id,p_production_lot_id,'weeding','monitor',true,case when v_weed_due then 'needs_attention' else 'satisfied' end,case when v_weed_due then null else v_today end,jsonb_build_object('source','field_care_state','current_need',v_weed_due))
on conflict(production_lot_id,care_kind) do update set policy_status='monitor',required_before_harvest=true,current_status=case when v_weed_due then 'needs_attention' else 'satisfied' end,last_satisfied_at=case when v_weed_due then atlas.production_care_policies.last_satisfied_at else v_today end,metadata=atlas.production_care_policies.metadata||jsonb_build_object('source','field_care_state','current_need',v_weed_due),updated_at=now();
end;$$;
...
v_expected integer:=0;v_established integer:=0;v_alive numeric:=0;v_required_policies integer:=0;v_satisfied_policies integer:=0;v_unsatisfied text;v_status text;v_blocker text;
v_owner_task atlas.tasks%rowtype;v_harvest_task atlas.tasks%rowtype;v_visibility text:='assigned_worker';v_membership uuid;v_transition jsonb;v_today date:=(now() at time zone 'America/Chicago')::date;
begin
...
on conflict(production_lot_id) do update set gate_status=case when atlas.production_harvest_gates.gate_status in ('harvest_watch','harvest_ready') and excluded.gate_status='ready_for_watch' then atlas.production_harvest_gates.gate_status else excluded.gate_status end,
...
ready_at=case when excluded.gate_status='ready_for_watch' then coalesce(atlas.production_harvest_gates.ready_at,now()) else null end,refresh_version=atlas.production_harvest_gates.refresh_version+1,
metadata=atlas.production_harvest_gates.metadata||excluded.metadata,updated_at=now() returning * into v_gate;
if v_status='waiting_rules' then
 ...
 if v_owner_task.id is null then
  ...
  values(v_lot.farm_id,'Owner — Set pinch + harvest rules — '||v_lot.lot_label,'owner_decision','open','high',v_today,'production_harvest_gate',v_gate.id,
...
if v_status='ready_for_watch' then
 select * into v_harvest_task from atlas.tasks where id=coalesce(v_gate.harvest_readiness_task_id,v_gate.harvest_task_id);
 if v_harvest_task.id is null then
  ...
  values(v_lot.farm_id,'Open harvest readiness — '||v_lot.lot_label,'production_harvest_readiness','open','high',v_rule.harvest_watch_start,'production_harvest_gate',v_gate.id,
  ...
  update atlas.production_harvest_gates set harvest_readiness_task_id=v_harvest_task.id,harvest_task_id=v_harvest_task.id,gate_status='harvest_watch' where id=v_gate.id;
  ...
  insert into atlas.task_objects(task_id,object_id,role) select v_harvest_task.id,object_id,'target' from atlas.production_field_stands where production_lot_id=v_lot.id and current_plants>0 and stand_status<>'cleared' on conflict do nothing;
  insert into atlas.task_crop_cycles(task_id,crop_cycle_id,role,confidence,source,metadata) select v_harvest_task.id,crop_cycle_id,'observes','confirmed','production_stage_engine',jsonb_build_object('harvest_gate_id',v_gate.id) from atlas.production_field_stands where production_lot_id=v_lot.id and current_plants>0 and stand_status<>'cleared' on conflict do nothing;
