...
BEGIN
  ...
  THEN
    ...
    INTO v_object_id
    FROM atlas.weed_cards card
    WHERE card.id = NEW.generated_from_id;
  ELSIF v_object_id IS NULL
    AND NEW.generated_from = 'rhythm_clock'
    AND NEW.generated_from_id IS NOT NULL
  THEN
    SELECT state.subject_id
    INTO v_object_id
    FROM atlas.rhythm_state state
    WHERE state.id = NEW.generated_from_id
  ...
  IF v_object_id IS NULL AND NEW.planned_occurrence_id IS NOT NULL THEN
    ...
    INTO v_occurrence_source_kind, v_occurrence_source_id
    ...
    IF v_occurrence_source_kind = 'maintenance_weeding_collection' THEN
      ...
      INTO v_object_id
      FROM atlas.maintenance_objects maintenance
      WHERE maintenance.id = v_occurrence_source_id;
    ELSIF v_occurrence_source_kind = 'rhythm_state' THEN
      SELECT state.subject_id
      INTO v_object_id
      FROM atlas.rhythm_state state
      WHERE state.id = v_occurrence_source_id
        AND state.rhythm_key = 'weed_stewardship'
        AND state.subject_kind = 'growing_object';
    END IF;
  END IF;
...
BEGIN
  ...
  SELECT pass.id, pass.current_condition
  INTO v_pass_id, v_condition_before
  ...
  SELECT coalesce(array_agg(task.id ORDER BY task.created_at), '{}'::uuid[])
  INTO v_task_ids
  ...
  IF v_pass_id IS NOT NULL THEN
    INSERT INTO atlas.weed_sessions (
      ...
      0,
      false,
      coalesce(v_condition_before, 'clear'),
      'clear',
  ...
  IF v_state.id IS NOT NULL THEN
    IF v_state.current_occurrence_id IS NOT NULL THEN
      ...
      SET state = 'completed',
          ...
          updated_at = now()
      WHERE occurrence.id = v_state.current_occurrence_id
...
WHERE (
  lower(coalesce(task.action_key, '')) IN ('weed', 'weeding')
  OR lower(coalesce(task.metadata ->> 'work_route', '')) IN ('weed', 'weeding')
  OR coalesce(task.metadata ->> 'weed_card_managed', 'false') = 'true'
  OR coalesce(task.metadata ->> 'maintenance_type', '') = 'weed'
);

UPDATE atlas.planned_work_occurrences occurrence
SET title = occurrence.title
WHERE occurrence.source_kind IN ('maintenance_weeding_collection', 'rhythm_state');
...
BEGIN
  ...
  IF v_object_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Field Row 8 Weed Card did not resolve to clear.';
  END IF;
