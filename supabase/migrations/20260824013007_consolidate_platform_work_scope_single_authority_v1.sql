...
revoke all on function atlas.work_scope_resolve_v1(uuid,uuid) from public,anon,authenticated,service_role;
drop function atlas.work_scope_resolve_v1(uuid,uuid);

delete from atlas.authenticated_rpc_registry
where signature='atlas.work_scope_resolve_v1(uuid, uuid)';

delete from atlas.productization_surface_classifications
where family_key='work_scope';
