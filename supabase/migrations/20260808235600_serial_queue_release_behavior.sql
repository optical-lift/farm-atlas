...
begin
  ...
  update atlas.tasks task set metadata=coalesce(task.metadata,'{}'::jsonb)||jsonb_build_object(
    ...
    'release_queue_policy','completion_gated_serial','release_queue_active_count',coalesce(v_active_count,0),
    'release_queue_queued_count',coalesce(v_queued_count,0),'release_queue_completed_count',coalesce(v_completed_count,0),
    'release_queue_next_label',coalesce(v_next_label,''),'release_queue_summary_updated_at',now()),updated_at=now()
...
begin
  ...
  if v_release_timing='same_day' then v_due_date:=v_completed_date; else v_due_date:=v_completed_date+1; if extract(dow from v_due_date)=0 then v_due_date:=v_due_date+1; end if; end if;
  update atlas.planned_work_occurrences set planned_due_date=v_due_date,not_before_date=least(coalesce(not_before_date,v_due_date),v_due_date),gate_satisfied_at=now(),
    state=case when state in ('released','completed') then state else 'eligible' end,
    metadata=metadata||jsonb_build_object('release_queue_key',p_queue_key,'release_queue_position',v_next_item.position,'released_for_date',v_due_date,'queue_release_timing',v_release_timing),updated_at=now()
  ...
  if v_task_id is not null and exists(select 1 from atlas.tasks where id=v_task_id and status in ('open','blocked')) then
    update atlas.task_release_queue_items set task_id=v_task_id,planned_occurrence_id=v_occurrence_id,state='active',activated_at=now(),updated_at=now(),metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('released_after_completion',true,'released_for_date',v_due_date,'release_timing',v_release_timing) where id=v_next_item.id;
  else
    update atlas.task_release_queue_items set planned_occurrence_id=v_occurrence_id,task_id=null,state='queued',updated_at=now(),metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('release_waiting_on_capacity',true,'release_timing',v_release_timing) where id=v_next_item.id;
    v_task_id:=null;
  end if;
