...
begin
  ...
  v_approval_required := p_queue_key='anna_weeding_rotation'
    or coalesce((v_next_item.metadata->>'owner_schedule_approval_required')::boolean,false);
  ...
  set planned_due_date=v_due_date,
      ...
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
        ...
        'owner_schedule_approved_date',v_approved_date
      ),
      updated_at=now()
  ...
  if v_task_id is not null
    ...
    set task_id=v_task_id,
        ...
        activated_at=now(),
        updated_at=now(),
        metadata=(coalesce(metadata,'{}'::jsonb)-'awaiting_owner_schedule_approval')||jsonb_build_object(
          ...
          'owner_schedule_approval_consumed_at',now()
        )
    where id=v_next_item.id;
  else
    update atlas.task_release_queue_items
