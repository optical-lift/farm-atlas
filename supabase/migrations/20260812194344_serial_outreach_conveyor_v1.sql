...
-- Normalize the worker-facing vocabulary and serial sequence across all current outreach families.
...
set title=case t.metadata->>'task_key'
    ...
    task_type='network_outreach',
    action_key='call',
    metadata=coalesce(t.metadata,'{}'::jsonb)||jsonb_build_object(
      'checklist_mode','network_outreach',
      'operation_family','outreach',
      'operation_move','call',
      'display_action','Call',
      'work_route','network',
      'outreach_queue_key','anna_outreach_conveyor',
      'outreach_conveyor_version','serial_outreach_conveyor_v1'
    ),
    updated_at=now()
...
-- Only the first batch is presently released. Everything downstream waits without a due-date pile.
update atlas.tasks t
set status='blocked',
    due_date=null,
    blocker_text='Waiting for the previous outreach batch to be completed.',
    metadata=coalesce(t.metadata,'{}'::jsonb)||jsonb_build_object('outreach_release_state','queued','queued_at',now()),
    updated_at=now()
where t.metadata->>'task_key' in (
  'anna_20260810_find_free_woodchips_weed_suppression',
  'anna_restaurant_bud_vase_outreach_batch_1',
...
-- Downstream contact rows stay hidden with their parent until that parent is released.
update atlas.tasks child
set status='blocked',
    due_date=null,
    blocker_text='Waiting for this outreach batch to be released.',
    metadata=coalesce(child.metadata,'{}'::jsonb)||jsonb_build_object('outreach_release_state','queued','outreach_conveyor_version','serial_outreach_conveyor_v1'),
    updated_at=now()
...
-- Keep batch 1 active on Friday and normalize its children to the same execution date.
update atlas.tasks t
set status='open', due_date=date '2026-08-14', blocker_text=null,
    metadata=coalesce(t.metadata,'{}'::jsonb)||jsonb_build_object('outreach_release_state','released','execution_date','2026-08-14'),
    updated_at=now()
where t.metadata->>'task_key'='anna_florist_wholesale_batch_1_20260810' and t.status not in ('done','archived');

update atlas.tasks child
set status='open', due_date=date '2026-08-14', blocker_text=null,
    metadata=coalesce(child.metadata,'{}'::jsonb)||jsonb_build_object('outreach_release_state','released','execution_date','2026-08-14'),
    updated_at=now()
where child.parent_task_id=(select id from atlas.tasks where metadata->>'task_key'='anna_florist_wholesale_batch_1_20260810' order by created_at desc limit 1)
  and child.status not in ('done','archived');

-- Make restaurant/church child vocabulary actual calls instead of generic Checklist rows.
update atlas.tasks child
set title=case
      when parent.metadata->>'task_key'='anna_restaurant_bud_vase_outreach_batch_1' then regexp_replace(child.title,'^Checklist\s*[—-]\s*','Call ')||' — weekly bud vases'
      when parent.metadata->>'task_key'='network_20260725_call_10_churches' then regexp_replace(child.title,'^Checklist\s*[—-]\s*','Call ')||' — Thursdays at Elm'
      else child.title end,
    task_type='network_outreach_contact',
    action_key='call',
    metadata=coalesce(child.metadata,'{}'::jsonb)||jsonb_build_object('operation_family','outreach','operation_move','call','display_action','Call'),
    updated_at=now()
from atlas.tasks parent
where child.parent_task_id=parent.id
  and parent.metadata->>'task_key' in ('anna_restaurant_bud_vase_outreach_batch_1','network_20260725_call_10_churches');

-- Serial release happens onto the next available worker day, never immediately into the same day.
...
begin
  ...
  if p_effective_membership_id is distinct from v_task.assigned_membership_id and v_role not in ('owner','manager') then
    raise exception using errcode='42501',message='This outreach batch belongs to another worker.';
  end if;

  if coalesce(v_task.metadata->>'checklist_mode','') <> 'network_outreach'
     and coalesce(v_task.metadata->>'operation_family','') <> 'outreach' then
    raise exception using errcode='22023',message='This is not an outreach batch.';
  end if;
  ...
  select * into v_next
  from atlas.tasks t
  where t.farm_id=v_task.farm_id and t.metadata->>'task_key'=p_next_task_key
  ...
  set status='open',
      ...
      release_reason='previous_outreach_batch_complete',
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
        ...
        'released_for_next_worker_day',true
      ),
      updated_at=v_now
  where id=v_next.id;

  update atlas.tasks
  set status='open', due_date=v_release_date, blocker_text=null,
      released_at=coalesce(released_at,v_now),
      release_reason=coalesce(release_reason,'parent_outreach_batch_released'),
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('outreach_release_state','released','execution_date',v_release_date),
      updated_at=v_now
  where parent_task_id=v_next.id and status='blocked';
  get diagnostics v_released_children=row_count;

  update atlas.planned_work_occurrences pwo
  set planned_due_date=v_release_date,
      not_before_date=v_release_date,
      metadata=coalesce(pwo.metadata,'{}'::jsonb)||jsonb_build_object('outreachExecutionDate',v_release_date,'outreachReleasedFromTaskId',v_task.id),
      updated_at=v_now
  where pwo.id=v_next.planned_occurrence_id;

  v_idempotency_key:=v_next.id::text||':network-outreach-release:'||v_task.id::text;
  insert into atlas.task_transitions(farm_id,task_id,transition,previous_status,next_status,action_key,work_class,reason,idempotency_key,payload,created_by,actor_membership_id,actor_role,created_at)
  values(v_task.farm_id,v_next.id,'released',v_next.status,'open','call',coalesce(v_next.work_class,'standard'),'Previous outreach batch completed',v_idempotency_key,jsonb_build_object('source_task_id',v_task.id,'release_source','outreach_conveyor','released_children',v_released_children,'release_date',v_release_date),'outreach_conveyor',p_effective_membership_id,v_role,v_now)
