...
set metadata=coalesce(t.metadata,'{}'::jsonb)||jsonb_build_object(
      ...
      'operation_family','build',
      'operation_move','repair',
      'normalized_by','aug17_worker_execution_normalization_v1',
      'normalized_at',now()
    ),
    updated_at=now()
...
set task_payload=jsonb_set(
      coalesce(p.task_payload,'{}'::jsonb),
      '{metadata}',
      coalesce(p.task_payload->'metadata','{}'::jsonb)||
    ...
    metadata=coalesce(p.metadata,'{}'::jsonb)||jsonb_build_object('normalizedBy','aug17_worker_execution_normalization_v1','normalizedAt',now()),
    updated_at=now()
from atlas.tasks t
